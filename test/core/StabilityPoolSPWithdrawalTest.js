  ...
  describe('Stability Pool Withdrawal', async () => {
    ...
    it('withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(100000, 18)),
        whale,
        whale,
      ...
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(alice)).toString(),
      ...
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(bob)).toString(),
          toBN('6666666666666666666666').mul(toBN(2))
        ),
        20000
      )
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(carol)).toString(),
          toBN('6666666666666666666666').mul(toBN(2))
        ),
        20000
      )
    ...
    it('withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations', async () => {
      ...
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(alice)).toString(), '0'), 10000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(bob)).toString(), '0'), 10000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(carol)).toString(), '0'), 10000)
    ...
    it('withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing SLSD', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing SLSD', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1000000, 18)),
        whale,
        whale,
    ...
    it('withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(alice)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(bob)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(carol)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(dennis)).toString(), '0'), 100000)
    ...
    it('withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 1m ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      */
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(25000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(40000, 18)),
        defaulter_4,
        defaulter_4,
    ...
    it('withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(alice)).toString(), '0'), 1000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(bob)).toString(), '0'), 1000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(carol)).toString(), '0'), 1000)
    ...
    it('withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 SLSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool', async () => {
      ...
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(erin)).toString(),
    ...
    it('withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
      ...
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(carol)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(dennis)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(erin)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(flyn)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(graham)).toString(), '0'), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(harriet)).toString(), '0'), 100000)
    ...
    it('withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('9999999910000000000000'),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      ...
      // Expect Bob to withdraw 1% of initial deposit (100 SLSD) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(bob)).toString(),
    ...
    it('withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('59400000000000000000000'),
        defaulter_2,
        defaulter_2,
      ...
      */
      assert.isAtMost(
        th.getDifference((await SLSDToken.balanceOf(bob)).toString(), toBN(dec(100, 18)).mul(toBN(2))),
        100000
      )
      assert.isAtMost(
        th.getDifference((await SLSDToken.balanceOf(carol)).toString(), toBN(dec(200, 18)).mul(toBN(2))),
        100000
      )
      assert.isAtMost(
        th.getDifference((await SLSDToken.balanceOf(dennis)).toString(), toBN(dec(300, 18)).mul(toBN(2))),
        100000
      )
    ...
    it('withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(99999, 17)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      ...
      assert.isAtMost(
        th.getDifference((await SLSDToken.balanceOf(bob)).toString(), toBN(dec(1, 17)).mul(toBN(2))),
        100000
      )
    ...
    it('withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('59999400000000000000000'),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(bob)).toString(), dec(1, 17)), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(carol)).toString(), dec(2, 17)), 100000)
      assert.isAtMost(th.getDifference((await SLSDToken.balanceOf(dennis)).toString(), dec(3, 17)), 100000)
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('9999999999900000000000'),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      ...
      const aliceDeposit = (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString()
      const aliceDepositERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString()
    ...
    */
    it('withdrawFromSP(): Several deposits of 10000 SLSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      await SLSDToken.transfer(dennis, toBN(dec(99999, 17)).mul(toBN(2)), { from: whale })
      ...
      assert.equal(await SLSDToken.balanceOf(alice), '0')
      assert.equal(await SLSDToken.balanceOf(alice), '0')
      assert.equal(await SLSDToken.balanceOf(alice), '0')
      // D should withdraw around 0.9999 SLSD, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await SLSDToken.balanceOf(dennis)).toString(), toBN(dec(99999, 12)).mul(toBN(2))),
        100000
      )
    ...
    it('withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24100, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(200, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24300, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2, value: dec(200, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24500, 18)),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3, value: dec(200, 'ether') }
      )
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 9999.9999999 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      // E deposits 9999.9 SLSD
      await SLSDToken.transfer(erin, dec(99999, 17), { from: whale })
    ...
    it('withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1, 36)),
        defaulter_1,
        defaulter_1,
      ...
      // Check SLSD balances
      const aliceSLSDBalance = await SLSDToken.balanceOf(alice)
      const aliceExpectedSLSDBalance = web3.utils.toBN(dec(5, 35))
      const aliceSLSDBalDiff = aliceSLSDBalance.sub(aliceExpectedSLSDBalance.mul(toBN(2))).abs()

      assert.isTrue(aliceSLSDBalDiff.lte(toBN(dec(1, 18)))) // error tolerance of 1e18

      const bobSLSDBalance = await SLSDToken.balanceOf(bob)
      const bobExpectedSLSDBalance = toBN(dec(5, 35))
      const bobSLSDBalDiff = bobSLSDBalance.sub(bobExpectedSLSDBalance.mul(toBN(2))).abs()
    ...
    it('withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price', async () => {
      ...
      const aliceSLSDBalance = await SLSDToken.balanceOf(alice)
      const aliceExpectedSLSDBalance = toBN('99999999999999997500000000000000000000').mul(toBN(2))
      const aliceSLSDBalDiff = aliceSLSDBalance.sub(aliceExpectedSLSDBalance).abs()

      assert.isTrue(aliceSLSDBalDiff.lte(toBN(dec(1, 18))))

      const bobSLSDBalance = await SLSDToken.balanceOf(bob)
      const bobExpectedSLSDBalance = toBN('99999999999999997500000000000000000000').mul(toBN(2))
