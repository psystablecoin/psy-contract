...
contract('TroveManager', async (accounts) => {
  ...
  const withdrawSLSD = async (params) => th.withdrawSLSD(contracts, params)
  ...
  beforeEach(async () => {
    ...
    await psyToken.unprotectedMint(multisig, dec(1, 24))
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    assert.equal((await psyParameters.MCR(ZERO_ADDRESS)).toString(), '1100000000000000000')
    assert.equal((await psyParameters.MCR(erc20.address)).toString(), '1100000000000000000')

    // Alice increases debt to 180 SLSD, lowering her ICR to 1.11
    ...
    await withdrawSLSD({ ICR: targetICR, extraParams: { from: alice } })
    await withdrawSLSD({ asset: erc20.address, ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100SLSD, reducing Alice's ICR below MCR
  ...
  it('liquidate(): decreases ActivePool ETH and SLSDDebt by correct amounts', async () => {
    // --- SETUP ---
    ...
    // check ActivePool ETH and SLSD debt before
    ...
    const activePooL_SLSDDebt_Before = (await activePool.getSLSDDebt(ZERO_ADDRESS)).toString()
    ...
    const activePooL_SLSDDebt_Before_Asset = (await activePool.getSLSDDebt(erc20.address)).toString()
    ...
    th.assertIsApproximatelyEqual(activePooL_SLSDDebt_Before, A_totalDebt.add(B_totalDebt))
    ...
    th.assertIsApproximatelyEqual(activePooL_SLSDDebt_Before_Asset, A_totalDebt_Asset.add(B_totalDebt_Asset))

    // price drops to 1ETH:100SLSD, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000')
    ...
    /* close Bob's Trove. Should liquidate his ether and SLSD,
    leaving Alice’s ether and SLSD debt in the ActivePool. */
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })

    // check ActivePool ETH and SLSD debt
    ...
    const activePooL_SLSDDebt_After = (await activePool.getSLSDDebt(ZERO_ADDRESS)).toString()
    ...
    const activePooL_SLSDDebt_After_Asset = (await activePool.getSLSDDebt(erc20.address)).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePooL_SLSDDebt_After, A_totalDebt)
    ...
    th.assertIsApproximatelyEqual(activePooL_SLSDDebt_After_Asset, A_totalDebt_Asset)
  })

  it('liquidate(): increases DefaultPool ETH and SLSD debt by correct amounts', async () => {
    // --- SETUP ---
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(21, 17)),
    ...
    // check DefaultPool ETH and SLSD debt before
    ...
    const defaultPooL_SLSDDebt_Before = (await defaultPool.getSLSDDebt(ZERO_ADDRESS)).toString()
    ...
    const defaultPooL_SLSDDebt_Before_Asset = (await defaultPool.getSLSDDebt(erc20.address)).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPooL_SLSDDebt_Before, '0')

    assert.equal(defaultPool_ETH_Before_Asset, '0')
    assert.equal(defaultPool_RawEther_Before_Asset, '0')
    assert.equal(defaultPooL_SLSDDebt_Before_Asset, '0')
    ...
    const defaultPooL_SLSDDebt_After = (await defaultPool.getSLSDDebt(ZERO_ADDRESS)).toString()
    ...
    const defaultPooL_SLSDDebt_After_Asset = (await defaultPool.getSLSDDebt(erc20.address)).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPooL_SLSDDebt_After, B_totalDebt)
    ...
    th.assertIsApproximatelyEqual(defaultPooL_SLSDDebt_After_Asset, B_totalDebt_Asset)
  })
  ...
  it('liquidate(): updates the snapshots of total stakes and total collateral', async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and SLSD should be added to the DefaultPool.
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })
  ...
  it('liquidate(): updates the L_ETH and L_SLSDDebt reward-per-unit-staked totals', async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(8, 18)),
      extraParams: { from: alice },
    ...
    // price drops to 1ETH:100SLSD, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000')
    ...
    // Carol's ether*0.995 and SLSD should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
    const L_SLSDDebt_AfterCarolLiquidated = await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)
    const L_ETH_AfterCarolLiquidated_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
    const L_SLSDDebt_AfterCarolLiquidated_Asset = await troveManagerHelpers.L_SLSDDebts(erc20.address)

    const L_ETH_expected_1 = th
      .applyLiquidationFee(C_collateral)
      .mul(mv._1e18BN)
      .div(A_collateral.add(B_collateral))
    const L_SLSDDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_ETH_expected_1_Asset = th
      .applyLiquidationFee(C_collateral_Asset)
      .mul(mv._1e18BN)
      .div(A_collateral_Asset.add(B_collateral_Asset))
    const L_SLSDDebt_expected_1_Asset = C_totalDebt_Asset.mul(mv._1e18BN).div(
    ...
    assert.isAtMost(th.getDifference(L_SLSDDebt_AfterCarolLiquidated, L_SLSDDebt_expected_1), 100)
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated_Asset, L_ETH_expected_1_Asset), 100)
    assert.isAtMost(th.getDifference(L_SLSDDebt_AfterCarolLiquidated_Asset, L_SLSDDebt_expected_1_Asset), 100)

    // Bob now withdraws SLSD, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawSLSD({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob },
    })
    const { increasedTotalDebt: B_increasedTotalDebt_Asset } = await withdrawSLSD({
      asset: erc20.address,
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob },
    })
    ...
    // price drops to 1ETH:50SLSD, reducing Bob's ICR below MCR
   ...
   L_SLSDDebt = (180 / 20) + (890 / 10) = 98 SLSD */
    const L_ETH_AfterBobLiquidated = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
    const L_SLSDDebt_AfterBobLiquidated = await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)
    ...
    const L_SLSDDebt_expected_2 = L_SLSDDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(B_collateral.mul(L_SLSDDebt_expected_1).div(mv._1e18BN))
        .mul(mv._1e18BN)
        .div(A_collateral)
    )
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_SLSDDebt_AfterBobLiquidated, L_SLSDDebt_expected_2), 100)
  })
  ...
  it('liquidate(): Liquidates undercollateralized trove if there are two troves in the system', async () => {
    ...
    // Alice creates a single trove with 0.7 ETH and a debt of 70 SLSD, and provides 10 SLSD to SP
    await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 SLSD to SP
  ...
  it('liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves', async () => {
    // Whale provides SLSD to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(4, 18)),
      extraSLSDAmount: spDeposit,
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraSLSDAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(3, 18)),
      extraSLSDAmount: spDeposit,
    ...
    await SLSDToken.transfer(dennis, spDeposit.mul(toBN(2)), { from: bob })

    //Dennis provides SLSD to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString()
    const dennis_ETHGain_Before = (await stabilityPool.getDepositorAssetGain(dennis)).toString()
    const dennis_Deposit_Before_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString()
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString()
    const dennis_ETHGain_After = (await stabilityPool.getDepositorAssetGain(dennis)).toString()

    const dennis_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    const bob_Deposit_Before = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()
    const bob_ETHGain_Before = (await stabilityPool.getDepositorAssetGain(bob)).toString()

    const bob_Deposit_Before_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
    ...
    const bob_Deposit_After = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()
    const bob_ETHGain_After = (await stabilityPool.getDepositorAssetGain(bob)).toString()

    const bob_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraSLSDAmount: A_spDeposit, extraParams: { from: alice } })

    await openTrove({ asset: erc20.address, ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(8, 18)),
      extraSLSDAmount: A_spDeposit,
      extraParams: { from: alice },
    })

    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(218, 16)),
      extraSLSDAmount: B_spDeposit,
      extraParams: { from: bob },
    })
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
    const bob_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(bob)
    const bob_ETHGain_Before = await stabilityPool.getDepositorAssetGain(bob)

    const bob_Deposit_Before_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
    ...
    /* Alice's SLSD Loss = (300 / 400) * 200 = 150 SLSD
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's SLSDLoss = (100 / 400) * 200 = 50 SLSD
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 SLSD, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString()
    const alice_ETHGain_After = (await stabilityPool.getDepositorAssetGain(alice)).toString()

    const alice_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString()
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedSLSDDeposit(bob)
    const bob_ETHGain_After = await stabilityPool.getDepositorAssetGain(bob)

    const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { SLSDAmount: A_SLSDAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    })
    const { SLSDAmount: B_SLSDAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    })
    const { SLSDAmount: C_SLSDAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    })
    ...
    await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { SLSDAmount: A_SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    })
    const { SLSDAmount: B_SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    })
    const { SLSDAmount: C_SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    ...
    // Liquidate A, B and C
    await activePool.getSLSDDebt(ZERO_ADDRESS)
    await defaultPool.getSLSDDebt(ZERO_ADDRESS)
    await activePool.getSLSDDebt(erc20.address)
    await defaultPool.getSLSDDebt(erc20.address)
    ...
    assert.equal((await SLSDToken.balanceOf(alice)).toString(), A_SLSDAmount.add(A_SLSDAmount_Asset))
    assert.equal((await SLSDToken.balanceOf(bob)).toString(), B_SLSDAmount.add(B_SLSDAmount_Asset))
    assert.equal((await SLSDToken.balanceOf(carol)).toString(), C_SLSDAmount.add(C_SLSDAmount_Asset))
  })
  ...
  it('liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    // Defaulter opens with 60 SLSD, 0.6 ETH
    ...
    /* Liquidate defaulter. 30 SLSD and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 SLSD, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 SLSD, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 SLSD, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
    await troveManager.liquidate(erc20.address, defaulter_1)
  ...
  it('liquidate(): when SP > 0, triggers PSY reward event - increases the sum G', async () => {
    ...
    assert.equal(await stabilityPool.getTotalSLSDDeposits(), dec(100, 18))

    await stabilityPoolERC20.provideToSP(dec(100, 18), { from: B })
    assert.equal(await stabilityPoolERC20.getTotalSLSDDeposits(), dec(100, 18))
    ...
    // Price drops to 1ETH:100SLSD, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18))
    await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
    assert.isFalse(await th.checkRecoveryMode(contracts, erc20.address))
    ...
    // Expect G has increased from the PSY reward event triggered
  ...
  it("liquidate(): when SP is empty, doesn't update G", async () => {
    ...
    // Check SP is empty
    assert.equal(await stabilityPool.getTotalSLSDDeposits(), '0')
    assert.equal(await stabilityPoolERC20.getTotalSLSDDeposits(), '0')
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 SLSD to the SP, but less than C's debt
    ...
    await borrowerOperations.repaySLSD(ZERO_ADDRESS, dec(1, 18), D, D, { from: D })
    await borrowerOperations.repaySLSD(ZERO_ADDRESS, dec(1, 18), E, E, { from: E })
    await borrowerOperations.repaySLSD(erc20.address, dec(1, 18), D, D, { from: D })
    await borrowerOperations.repaySLSD(erc20.address, dec(1, 18), E, E, { from: E })
    ...
    const pendingSLSDDebt_C = await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, C)
    const defaultPoolETH = await defaultPool.getAssetBalance(ZERO_ADDRESS)
    const defaultPoolSLSDDebt = await defaultPool.getSLSDDebt(ZERO_ADDRESS)

    const pendingETH_C_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, C)
    const pendingSLSDDebt_C_Asset = await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, C)
    const defaultPoolETH_Asset = await defaultPool.getAssetBalance(erc20.address)
    const defaultPoolSLSDDebt_Asset = await defaultPool.getSLSDDebt(erc20.address)

    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingSLSDDebt_C.lte(defaultPoolSLSDDebt))

    assert.isTrue(pendingETH_C_Asset.lte(defaultPoolETH_Asset))
    assert.isTrue(pendingSLSDDebt_C_Asset.lte(defaultPoolSLSDDebt_Asset))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingSLSDDebt_C, defaultPoolSLSDDebt), 1000)

    assert.isAtMost(th.getDifference(pendingETH_C_Asset, defaultPoolETH_Asset), 1000)
    assert.isAtMost(th.getDifference(pendingSLSDDebt_C_Asset, defaultPoolSLSDDebt_Asset), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100SLSD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18))
    const price = await priceFeed.getPrice()
  ...
  it('liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    // Liquidate defaulter. 30 SLSD and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 SLSD, 0.1 ETH
    await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
    await troveManager.liquidate(erc20.address, defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraSLSDAmount: dec(1, 24), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(10, 18)),
      extraSLSDAmount: dec(1, 24),
      extraParams: { from: whale },
    })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await SLSDToken.balanceOf(dennis)
    const E_balanceBefore = await SLSDToken.balanceOf(erin)
    const F_balanceBefore = await SLSDToken.balanceOf(flyn)
    ...
    assert.equal((await SLSDToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await SLSDToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await SLSDToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it('liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR', async () => {
    // Whale provides 500 SLSD to SP
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraSLSDAmount: toBN(dec(500, 18)),
      extraParams: { from: whale },
    })
    await openTrove({
      asset: erc20.address,
    ...
    // Check pool has 500 SLSD
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), dec(500, 18))
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), '0')
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain', async () => {
    // Whale provides 400 SLSD to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraSLSDAmount: whaleDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraSLSDAmount: whaleDeposit,
      extraParams: { from: whale },
    })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: A_deposit,
      extraParams: { from: alice },
    })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: B_deposit,
      extraParams: { from: bob },
    })
    const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
    ...
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), totalDeposits)
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), totalDeposits)
    ...
    Total SLSD in Pool: 800 SLSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 SLSD
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale SLSD loss: 650 * (400/800) = 325 SLSD
    alice SLSD loss:  650 *(100/800) = 81.25 SLSD
    bob SLSD loss: 650 * (300/800) = 243.75 SLSD

    whale remaining deposit: (400 - 325) = 75 SLSD
    alice remaining deposit: (100 - 81.25) = 18.75 SLSD
    bob remaining deposit: (300 - 243.75) = 56.25 SLSD
    ...
    Total remaining deposits: 150 SLSD
    Total ETH gain: 4.975 ETH */

    // Check remaining SLSD Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedSLSDDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedSLSDDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedSLSDDeposit(bob)
    ...
    const whale_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(whale)
    const alice_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
    const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
    ...
    const total_SLSDinSP = (await stabilityPool.getTotalSLSDDeposits()).toString()
    const total_ETHinSP = (await stabilityPool.getAssetBalance()).toString()

    const total_SLSDinSP_Asset = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
    const total_ETHinSP_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

    assert.isAtMost(th.getDifference(total_SLSDinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
    assert.isAtMost(
      th.getDifference(total_SLSDinSP_Asset, totalDeposits_Asset.sub(liquidatedDebt_Asset)),
      1000
    )
    assert.isAtMost(th.getDifference(total_ETHinSP_Asset, th.applyLiquidationFee(liquidatedColl_Asset)), 1000)
  })

  it('liquidateTroves(): when SP > 0, triggers PSY reward event - increases the sum G', async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraSLSDAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100SLSD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18))
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): skips if trove is non-existent', async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraSLSDAmount: spDeposit,
    ...
    th.assertIsApproximatelyEqual(
      (await stabilityPool.getTotalSLSDDeposits()).toString(),
      spDeposit.sub(A_debt).sub(B_debt)
    )
    th.assertIsApproximatelyEqual(
      (await stabilityPoolERC20.getTotalSLSDDeposits()).toString(),
  ...
  it('batchLiquidateTroves(): skips if a trove has been closed', async () => {
    ...
    await SLSDToken.transfer(carol, dec(200, 18), { from: whale })
  ...
  it('batchLiquidateTroves: when SP > 0, triggers PSY reward event - increases the sum G', async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraSLSDAmount: partialRedemptionAmount,
      extraParams: { from: alice },
    })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra SLSD. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided SLSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraSLSDAmount: dec(10, 18),
      extraParams: { from: alice },
    })
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(290, 16)),
      extraSLSDAmount: dec(8, 18),
      extraParams: { from: bob },
    })
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraSLSDAmount: redemptionAmount,
      extraParams: { from: dennis },
    })
    await openTrove({
      asset: erc20.address,
    ...
    const dennis_SLSDBalance_Before = await SLSDToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 SLSD
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      redemptionAmount,
      price,
    ...
    /* check that Dennis' redeemed 20 SLSD has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) SLSD debt + 50 for gas compensation. */
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount SLSD to ETH, at ETH:USD price 200
    ...
    const dennis_SLSDBalance_After = (await SLSDToken.balanceOf(dennis)).toString()
    assert.equal(
      dennis_SLSDBalance_After,
      dennis_SLSDBalance_Before.sub(redemptionAmount).sub(redemptionAmount_Asset)
    )
  })
  ...
  it('redeemCollateral(): with invalid first hint, zero address', async () => {
    ...
    const expectedTotalETHDrawn_Asset = redemptionAmount_Asset.div(toBN(200)) // convert redemptionAmount SLSD to ETH, at ETH:USD price 200
    const expectedReceivedETH_Asset = expectedTotalETHDrawn_Asset.sub(toBN(ETHFee_Asset))
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 SLSD), 20000%
    const { SLSDAmount: F_SLSDAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraSLSDAmount: redemptionAmount.mul(toBN(2)),
      extraParams: { from: flyn },
    })
    const { SLSDAmount: F_SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 18)),
      extraSLSDAmount: redemptionAmount.mul(toBN(2)),
      extraParams: { from: flyn },
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 SLSD
    const flynBalance = await SLSDToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_SLSDAmount.add(F_SLSDAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
    )
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 SLSD
    const flynBalance = (await SLSDToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_SLSDAmount.add(F_SLSDAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
    )
  ...
  it('redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt', async () => {
    ...
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveSLSDAmount(dec(20000, 18), ZERO_ADDRESS),
      B,
      B,
    ...
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveSLSDAmount(dec(30000, 18), ZERO_ADDRESS),
      C,
      C,
    ...
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveSLSDAmount(dec(10000, 18), erc20.address),
      A,
      A,
      { from: A }
    )
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveSLSDAmount(dec(20000, 18), erc20.address),
      B,
      B,
      { from: B }
    )
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveSLSDAmount(dec(30000, 18), erc20.address),
      C,
      C,
      { from: C }
    )
    ...
    await SLSDToken.transfer(B, await SLSDToken.balanceOf(A), { from: A })
    await SLSDToken.transfer(B, await SLSDToken.balanceOf(C), { from: C })
    ...
    // SLSD redemption is 55000 US
    const SLSDRedemption = dec(55000, 18)
    await th.redeemCollateralAndGetTxObject(B, contracts, SLSDRedemption, ZERO_ADDRESS)
    await th.redeemCollateralAndGetTxObject(B, contracts, SLSDRedemption, erc20.address)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(
      ...
      0,
      th._100pct,
      await getOpenTroveSLSDAmount(dec(6000, 18), ZERO_ADDRESS),
      A,
      A,
      { from: A, value: dec(1000, 'ether') }
    )
    ...
    await borrowerOperations.openTrove(
      ...
      dec(1000, 'ether'),
      th._100pct,
      await getOpenTroveSLSDAmount(dec(6000, 18), erc20.address),
      A,
      A,
      { from: A }
    )
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    await openTrove({ ICR: toBN(dec(363, 16)), extraSLSDAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(344, 16)),
      extraSLSDAmount: dec(8, 18),
      extraParams: { from: bob },
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 SLSD from Carol's Trove
      await troveManager.redeemCollateral(
        ZERO_ADDRESS,
        frontRunRedepmtion,
        firstRedemptionHint,
    ...
    // Dennis tries to redeem 20 SLSD
    const redemptionTx = await troveManager.redeemCollateral(
      ZERO_ADDRESS,
      redemptionAmount,
      firstRedemptionHint,
    ...
    // Since Alice already redeemed 1 SLSD from Carol's Trove, Dennis was  able to redeem:
    //  - 9 SLSD from Carol's
    //  - 8 SLSD from Bob's
    // for a total of 17 SLSD.

    // Dennis calculated his hint for redeeming 2 SLSD from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 SLSD to fully complete his redemption of 20 SLSD.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted SLSD converted to ETH, at ETH:USD price 200
  ...
  it.skip('redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool', async () => {
    ...
    const amount = await getOpenTroveSLSDAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraSLSDAmount: amount, extraParams: { from: bob } })

    await openTrove({ asset: erc20.address, ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraSLSDAmount: amount,
      extraParams: { from: bob },
    })

    await SLSDToken.transfer(carol, amount.mul(toBN(2)), { from: bob })
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 SLSD to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)
    ...
    const carol_SLSDBalance_After = (await SLSDToken.balanceOf(carol)).toString()
    assert.equal(carol_SLSDBalance_After, '0')
  })
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { SLSDAmount: B_SLSDAmount, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraSLSDAmount: A_debt,
      extraParams: { from: bob },
    })
    ...
    const { SLSDAmount: B_SLSDAmount_Asset, totalDebt: B_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraSLSDAmount: A_debt,
      extraParams: { from: bob },
    ...
    await SLSDToken.transfer(carol, B_SLSDAmount.add(B_SLSDAmount_Asset), { from: bob })
  ...
  it('redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one', async () => {
    ...
    const { totalDebt: D_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(195, 16)),
      extraSLSDAmount: redemptionAmount_Asset,
      extraParams: { from: dennis },
    })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraSLSDAmount: dec(10, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(100, 18)),
      extraSLSDAmount: dec(10, 18),
  ...
  it('redeemCollateral(): reverts when argument _amount is 0', async () => {
    ...
    // Alice opens trove and transfers 500SLSD to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: dec(500, 18), extraParams: { from: alice } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: dec(500, 18),
      extraParams: { from: alice },
    })
    await SLSDToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it('redeemCollateral(): reverts if max fee > 100%', async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraSLSDAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSLSDAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSLSDAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraSLSDAmount: dec(40, 18), extraParams: { from: D } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(30, 18),
      extraParams: { from: C },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(40, 18),
      extraParams: { from: D },
    })
  ...
  it('redeemCollateral(): reverts if fee exceeds max fee percentage', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(80, 18),
      extraParams: { from: A },
    })
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(90, 18),
      extraParams: { from: B },
    })
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
    ...
    // SLSD redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedSLSDRedemption = expectedTotalSupply.div(toBN(10))
    const attemptedSLSDRedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, ZERO_ADDRESS, lessThan5pct),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, erc20.address, lessThan5pct),
      'Fee exceeded provided maximum'
    )
    ...
    // Max fee is 1%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, ZERO_ADDRESS, dec(1, 16)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, erc20.address, dec(1, 16)),
      'Fee exceeded provided maximum'
    )
    ...
    // Max fee is 3.754%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, ZERO_ADDRESS, dec(3754, 13)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, erc20.address, dec(3754, 13)),
      'Fee exceeded provided maximum'
    )
    ...
    // Max fee is 0.5%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, ZERO_ADDRESS, dec(5, 15)),
      'Fee exceeded provided maximum'
    )
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedSLSDRedemption, erc20.address, dec(5, 15)),
      'Fee exceeded provided maximum'
    )
  })
  ...
  it('redeemCollateral(): succeeds if fee is less than max fee percentage', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(9500, 18),
      extraParams: { from: A },
    })
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(395, 16)),
      extraSLSDAmount: dec(9000, 18),
      extraParams: { from: B },
    })
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(390, 16)),
      extraSLSDAmount: dec(10000, 18),
      extraParams: { from: C },
    })
    ...
    // SLSD redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedSLSDRedemption = expectedTotalSupply.div(toBN(10))
    const attemptedSLSDRedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedSLSDRedemption.mul(mv._1e18BN).div(price)
    const ETHDrawn_Asset = attemptedSLSDRedemption_Asset.mul(mv._1e18BN).div(price)
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(195, 16)),
      extraSLSDAmount: dec(200, 18),
      extraParams: { from: carol },
    })
    const { totalDebt: D_totalDebt } = await openTrove({
      ICR: toBN(dec(190, 16)),
      extraSLSDAmount: dec(400, 18),
      extraParams: { from: dennis },
    })
    ...
    // Alice opens trove and transfers SLSD to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: redemptionAmount,
      extraParams: { from: alice },
    ...
    await SLSDToken.transfer(erin, redemptionAmount.add(redemptionAmount_Asset), { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString()

    const bob_SPDeposit_before_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
    const carol_SPDeposit_before_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString()
    const dennis_SPDeposit_before_Asset = (
      await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)
    ).toString()
    ...
    // Check the remaining SLSD and ETH in Stability Pool after liquidation is non-zero
    const SLSDinSP = await stabilityPool.getTotalSLSDDeposits()
    const ETHinSP = await stabilityPool.getAssetBalance()

    const SLSDinSP_Asset = await stabilityPoolERC20.getTotalSLSDDeposits()
    const ETHinSP_Asset = await stabilityPoolERC20.getAssetBalance()

    assert.isTrue(SLSDinSP.gte(mv._zeroBN))
    assert.isTrue(ETHinSP.gte(mv._zeroBN))

    assert.isTrue(SLSDinSP_Asset.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString()

    const bob_SPDeposit_after_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
    const carol_SPDeposit_after_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString()
  ...
  it('redeemCollateral(): caller can redeem their entire SLSDToken balance', async () => {
    ...
    // Alice opens trove and transfers 400 SLSD to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: dec(400, 18),
      extraParams: { from: alice },
    ...
    await SLSDToken.transfer(erin, toBN(dec(400, 18)).mul(toBN(2)), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await SLSDToken.balanceOf(erin)
    ...
    // Erin attempts to redeem 400 SLSD
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      dec(400, 18),
      price,
    ...
    // Check activePool debt reduced by  400 SLSD
    const activePool_debt_after = await activePool.getSLSDDebt(ZERO_ADDRESS)
    assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

    const activePool_debt_after_Asset = await activePool.getSLSDDebt(erc20.address)
    ...
    */
    ...
    const erin_balance_after = (await SLSDToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's SLSD token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    })
    ...
    // Erin tries to redeem 1000 SLSD
    try {
    ...
    // Erin tries to redeem 239482309 SLSD
    try {
      ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
        ZERO_ADDRESS,
    ...
    // Erin tries to redeem 2^256 - 1 SLSD
  ...
  it('redeemCollateral(): value of issued ETH == face value of redeemed SLSD (assuming 1 SLSD has value of $1)', async () => {
    ...
    // Alice opens trove and transfers 1000 SLSD each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: dec(4990, 18),
      extraParams: { from: alice },
    })
    const { collateral: A_coll_Asset, totalDebt: A_totalDebt_Asset } = await openTrove({
      asset: erc20.address,
    ...
    await SLSDToken.transfer(erin, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
    await SLSDToken.transfer(flyn, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
    await SLSDToken.transfer(graham, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: dec(1590, 18),
      extraParams: { from: bob },
    })
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraSLSDAmount: dec(1090, 18),
      extraParams: { from: carol },
    })
    ...
    const _120_SLSD = '120000000000000000000'
    const _373_SLSD = '373000000000000000000'
    const _950_SLSD = '950000000000000000000'
    ...
    await ({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _120_SLSD, price, 0))
    ...
    /* 120 SLSD redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_1.toString(), activeETH_0.sub(toBN(_120_SLSD).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_1_Asset.toString(),
      activeETH_0_Asset.sub(toBN(_120_SLSD).mul(mv._1e18BN).div(price))
    )

    // Flyn redeems 373 SLSD
    ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      _373_SLSD,
      price,
    ...
    ;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _373_SLSD, price, 0))
    ...
    /* 373 SLSD redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_2.toString(), activeETH_1.sub(toBN(_373_SLSD).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_2_Asset.toString(),
      activeETH_1_Asset.sub(toBN(_373_SLSD).mul(mv._1e18BN).div(price))
    )

    // Graham redeems 950 SLSD
    ;({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      ZERO_ADDRESS,
      _950_SLSD,
      price,
    ...
    ;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
      await hintHelpers.getRedemptionHints(erc20.address, _950_SLSD, price, 0))
    ...
    /* 950 SLSD redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200,
    ...
    assert.equal(activeETH_3.toString(), activeETH_2.sub(toBN(_950_SLSD).mul(mv._1e18BN).div(price)))
    assert.equal(
      activeETH_3_Asset.toString(),
      activeETH_2_Asset.sub(toBN(_950_SLSD).mul(mv._1e18BN).div(price))
    )
  })
  ...
  it('redeemCollateral(): reverts if there is zero outstanding system debt', async () => {
    // --- SETUP --- illegally mint SLSD to Bob
    await SLSDToken.unprotectedMint(bob, toBN(dec(100, 18)).mul(toBN(2)))

    assert.equal(await SLSDToken.balanceOf(bob), toBN(dec(100, 18)).mul(toBN(2)).toString())
    ...
    // Bob tries to redeem his illegally obtained SLSD
    try {
      await troveManager.redeemCollateral(
        ZERO_ADDRESS,
        dec(100, 18),
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint SLSD to Bob
    await SLSDToken.unprotectedMint(bob, '202000000000000000000')

    assert.equal(await SLSDToken.balanceOf(bob), '202000000000000000000')
    ...
    th.assertIsApproximatelyEqual((await activePool.getSLSDDebt(ZERO_ADDRESS)).toString(), totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getSLSDDebt(erc20.address)).toString(), totalDebt_Asset)
    ...
    // Bob attempts to redeem his ill-gotten 101 SLSD, from a system that has 100 SLSD outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        ZERO_ADDRESS,
  ...
  it('redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed', async () => {
    ...
    const A_balanceBefore = await SLSDToken.balanceOf(A)
    const B_balanceBefore = await SLSDToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 SLSD
    assert.equal(await SLSDToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18)).mul(toBN(2))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 SLSD
    assert.equal(
      A_balanceBefore.sub(await SLSDToken.balanceOf(A)).toString(),
  ...
  it('redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to PSY staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 PSY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
    await psyStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check PSY Staking contract balance before is zero
    const PSYStakingBalance_Before = await web3.eth.getBalance(psyStaking.address)
    const PSYStakingBalance_Before_Asset = await erc20.balanceOf(psyStaking.address)
    assert.equal(PSYStakingBalance_Before, '0')
    assert.equal(PSYStakingBalance_Before_Asset, '0')
    ...
    // Check PSY Staking contract balance after is non-zero
    const PSYStakingBalance_After = toBN(await web3.eth.getBalance(psyStaking.address))
    assert.isTrue(PSYStakingBalance_After.gt(toBN('0')))

    const PSYStakingBalance_After_Asset = toBN(await erc20.balanceOf(psyStaking.address))
    assert.isTrue(PSYStakingBalance_After_Asset.gt(toBN('0')))
  })

  it('redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-PSY-staked in PSY Staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 PSY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
    await psyStaking.stake(dec(1, 18), { from: multisig })
  ...
  it('redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to PSY staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 PSY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
    await psyStaking.stake(dec(1, 18), { from: multisig })
    ...
    const PSYStakingBalance_Before = toBN(await web3.eth.getBalance(psyStaking.address))
    const PSYStakingBalance_Before_Asset = toBN(await erc20.balanceOf(psyStaking.address))
    ...
    // check PSY Staking balance has increased
    assert.isTrue(PSYStakingBalance_After.gt(PSYStakingBalance_Before))
    assert.isTrue(PSYStakingBalance_After_Asset.gt(PSYStakingBalance_Before_Asset))
  })

  it('redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-PSY-staked in the staking contract', async () => {
    // time fast-forwards 1 year, and multisig stakes 1 PSY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
    await psyStaking.stake(dec(1, 18), { from: multisig })
  ...
  it('redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer', async () => {
    ...
    // Check total SLSD supply
    const activeSLSD = await activePool.getSLSDDebt(ZERO_ADDRESS)
    const defaultSLSD = await defaultPool.getSLSDDebt(ZERO_ADDRESS)

    const activeSLSD_Asset = await activePool.getSLSDDebt(erc20.address)
    const defaultSLSD_Asset = await defaultPool.getSLSDDebt(erc20.address)

    const totalSLSDSupply = activeSLSD.add(defaultSLSD)
    const totalSLSDSupply_Asset = activeSLSD_Asset.add(defaultSLSD_Asset)
    th.assertIsApproximatelyEqual(totalSLSDSupply, totalDebt)
    th.assertIsApproximatelyEqual(totalSLSDSupply_Asset, totalDebt_Asset)
  ...
  it('redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove', async () => {
    ...
    // whale redeems 360 SLSD.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    // whale redeems SLSD.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, ZERO_ADDRESS)
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems SLSD.  Expect this to fully redeem A, B, C, and partially redeem 15 SLSD from D.
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { SLSDAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: dec(1, 24),
      extraParams: { from: alice },
    })
    ...
    const { SLSDAmount: SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: dec(1, 24),
      extraParams: { from: alice },
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, SLSDAmount, true, alice, alice, {
        from: alice,
        value: SLSDAmount.mul(mv._1e18BN).div(price),
      })
    }
    ...
    for (let i = 0; i < 2; i++) {
      // Find hints for redeeming
      const { 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
        await hintHelpers.getRedemptionHints(erc20.address, SLSDAmount_Asset, price, 0)
      ...
      await borrowerOperations.adjustTrove(
        erc20.address,
        SLSDAmount_Asset.mul(mv._1e18BN).div(price),
        th._100pct,
        0,
        SLSDAmount_Asset,
        true,
  ...
  it('getPendingSLSDDebtReward(): Returns 0 if there is no pending SLSDDebt reward', async () => {
    // Make some troves
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraSLSDAmount: dec(20, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(20, 18)), extraSLSDAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_SLSDDebt = await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)
    assert.equal(current_L_SLSDDebt, 0)

    const current_L_SLSDDebt_Asset = await troveManagerHelpers.L_SLSDDebts(erc20.address)
    assert.equal(current_L_SLSDDebt_Asset, 0)

    const carolSnapshot_L_SLSDDebt = (await troveManagerHelpers.getRewardSnapshots(ZERO_ADDRESS, carol))[1]
    assert.equal(carolSnapshot_L_SLSDDebt, 0)

    const carolSnapshot_L_SLSDDebt_Asset = (
      await troveManagerHelpers.getRewardSnapshots(erc20.address, carol)
    )[1]
    assert.equal(carolSnapshot_L_SLSDDebt_Asset, 0)

    const carol_PendingSLSDDebtReward = await troveManagerHelpers.getPendingSLSDDebtReward(
      ZERO_ADDRESS,
      carol
    )
    assert.equal(carol_PendingSLSDDebtReward, 0)

    const carol_PendingSLSDDebtReward_Asset = await troveManagerHelpers.getPendingSLSDDebtReward(
      erc20.address,
      carol
    )
    assert.equal(carol_PendingSLSDDebtReward_Asset, 0)
  })
  ...
  it('computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 SLSD', async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 SLSD', async () => {
    const price = dec(100, 18)
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 SLSD', async () => {
  ...
  it('computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 SLSD', async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it('getTroveStatus(): Returns status', async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraSLSDAmount: B_totalDebt, extraParams: { from: A } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(150, 16)),
      extraSLSDAmount: B_totalDebt,
      extraParams: { from: A },
    })

    // to be able to repay:
    await SLSDToken.transfer(B, B_totalDebt, { from: A })
    await SLSDToken.transfer(B, B_totalDebt_Asset, { from: A })
