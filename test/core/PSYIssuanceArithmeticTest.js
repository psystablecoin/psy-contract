...
const logPSYBalanceAndError = (PSYBalance_A, expectedPSYBalance_A) => {
  console.log(
    `Expected final balance: ${expectedPSYBalance_A}, \n
    Actual final balance: ${PSYBalance_A}, \n
    Abs. error: ${expectedPSYBalance_A.sub(PSYBalance_A)}`
  )
}

const repeatedlyIssuePSY = async (stabilityPool, timeBetweenIssuances, duration) => {
  const startTimestamp = th.toBN(await th.getLatestBlockTimestamp(web3))
  let timePassed = 0

  // while current time < 1 month from deployment, issue PSY every minute
  while (timePassed < duration) {
    await th.fastForwardTime(timeBetweenIssuances, web3.currentProvider)
    await stabilityPool._unprotectedTriggerPSYIssuance()
...
contract('PSY community issuance arithmetic tests', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPool.address, ETH_WEEKLY)
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPoolERC20.address, ERC_WEEKLY)
  })
  ...
  it('Give full supply after 4 weeks, forwards 4 weeks, totalIssuePSY should be max supply', async () => {
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPool.address, dec(32_000_000 / 4, 18))
    ...
    await communityIssuanceTester.unprotectedIssuePSY(stabilityPool.address)
    const totalIssued = await communityIssuanceTester.totalPSYIssued(stabilityPool.address)
    assert.equal(totalIssued.toString(), dec(32_000_000, 18))
  })

  it('Give full supply after 4 weeks, forwards 8 weeks, totalIssuePSY should be max supply', async () => {
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPool.address, dec(32_000_000 / 4, 18))
  ...
  it('Total PSY tokens issued is correct after a week', async () => {
    const distribution = toBN(dec(8_000_000, 18))
    const expectedReward = distribution

    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPool.address, distribution)
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPoolERC20.address, distribution)
    ...
    // Issue PSY
    await communityIssuanceTester.unprotectedIssuePSY(stabilityPool.address)
    const totalPSYIssued = await communityIssuanceTester.totalPSYIssued(stabilityPool.address)

    await communityIssuanceTester.unprotectedIssuePSY(stabilityPoolERC20.address)
    const totalPSYIssuedERC20 = await communityIssuanceTester.totalPSYIssued(stabilityPoolERC20.address)

    assert.isAtMost(th.getDifference(totalPSYIssued, expectedReward), 1000000000000000)
    assert.isAtMost(th.getDifference(totalPSYIssuedERC20, expectedReward), 1000000000000000)
  })

  it('Total PSY tokens issued is correct after a month', async () => {
    const distribution = toBN(dec(8_000_000, 18))
    await communityIssuanceTester.setWeeklyPSYDistribution(stabilityPool.address, distribution)
    await communityIssuanceTester.setWeeklyPSYDistribution(
      stabilityPoolERC20.address,
    ...
    assert.isAtMost(th.getDifference(totalPSYIssued, distribution.mul(toBN(4))), 1000000000000000)
    assert.isAtMost(th.getDifference(totalPSYIssuedERC20, distribution.mul(toBN(4))), 1000000000000000)
  })
})
