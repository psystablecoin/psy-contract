...
contract('StabilityPool', async (accounts) => {
  ...
  describe('Stability Pool Mechanisms', async () => {
    ...
    // --- provideToSP() ---
    // increases recorded SLSD at Stability Pool
    it('provideToSP(): increases the Stability Pool SLSD balance', async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraSLSDAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check SLSD balances after
      assert.equal(await stabilityPool.getTotalSLSDDeposits(), 200)
      assert.equal(await stabilityPoolERC20.getTotalSLSDDeposits(), 200)
    })
    ...
    it("provideToSP(): reduces the user's SLSD balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraSLSDAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check user's SLSD balance change
      const alice_SLSDBalance_After = await slsdToken.balanceOf(alice)
      assert.equal(alice_SLSDBalance_Before.sub(alice_SLSDBalance_After), '400')
    })

    it('provideToSP(): increases totalSLSDDeposits by correct amount', async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 SLSD to StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(2000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      assert.equal(await stabilityPool.getTotalSLSDDeposits(), dec(2000, 18))
      assert.equal(await stabilityPoolERC20.getTotalSLSDDeposits(), dec(2000, 18))
    })
    ...
    it('provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked', async () => {
      ...
      const whaleSLSD = (await slsdToken.balanceOf(whale)).div(toBN(2))
      await stabilityPool.provideToSP(whaleSLSD, { from: whale })
      await stabilityPoolERC20.provideToSP(whaleSLSD, { from: whale })
      ...
      const SPSLSD_Before = await stabilityPool.getTotalSLSDDeposits()
      const SPSLSD_BeforeERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      ...
      const SPSLSD_AfterERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.isTrue(SPSLSD_After.lt(SPSLSD_Before))
      assert.isTrue(SPSLSD_AfterERC20.lt(SPSLSD_BeforeERC20))
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 SLSD each
      await openTrove({
        extraSLSDAmount: 0,
        ICR: toBN(dec(2, 18)),
      ...
      // Alice makes deposit #1: 150 SLSD
      await openTrove({
        extraSLSDAmount: toBN(dec(250, 18)),
        ICR: toBN(dec(3, 18)),
        extraParams: { from: alice },
      })
      await openTrove({
      ...
      // 2 users with Trove with 180 SLSD drawn are closed
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
      ...
      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const alice_compoundedDeposit_1ERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      ...
      // Bob withdraws SLSD and deposits to StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(1000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const alice_compoundedDeposit_2ERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
    ...
    it('provideToSP(): reverts if user tries to provide more than their SLSD balance', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, 'ether') },
      ...
      const aliceSLSDbal = await slsdToken.balanceOf(alice)
      const bobSLSDbal = await slsdToken.balanceOf(bob)

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceSLSDbal.add(toBN(1)), {
        from: alice,
      })
      const aliceTxPromiseERC20 = stabilityPoolERC20.provideToSP(aliceSLSDbal.add(toBN(1)), {
        from: alice,
      })
      ...
      const bobTxPromise = stabilityPool.provideToSP(bobSLSDbal.add(toBN(dec(235534, 18))), {
        from: bob,
      })
      const bobTxPromiseERC20 = stabilityPoolERC20.provideToSP(bobSLSDbal.add(toBN(dec(235534, 18))), {
        from: bob,
      })
    ...
    it('provideToSP(): reverts if user tries to provide 2^256-1 SLSD, which exceeds their balance', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, 'ether') },
      ...
      // Alice attempts to deposit 2^256-1 SLSD
      try {
    ...
    it('provideToSP(): reverts if cannot receive ETH Gain', async () => {
      ...
      await slsdToken.transfer(nonPayable.address, dec(250, 18), { from: whale })
      await slsdToken.transfer(nonPayable.address, dec(250, 18), { from: whale })

      // NonPayable makes deposit #1: 150 SLSD
      ...
      // NonPayable tries to make deposit #2: 100SLSD (which also attempts to withdraw ETH gain)
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      const carol_SLSDDeposit_Before = (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString()
      ...
      const carol_SLSDDeposit_BeforeERC20 = (
        await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)
      ).toString()
      ...
      assert.equal((await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(), dec(1000, 18))

      await stabilityPoolERC20.provideToSP(dec(1000, 18), { from: dennis })
      assert.equal((await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString(), dec(1000, 18))
      ...
      const carol_SLSDDeposit_After = (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString()
      ...
      const carol_SLSDDeposit_AfterERC20 = (
        await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)
      ).toString()
      ...
      assert.equal(carol_SLSDDeposit_Before, carol_SLSDDeposit_After)
      ...
      assert.equal(carol_SLSDDeposit_BeforeERC20, carol_SLSDDeposit_AfterERC20)
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(), dec(1000, 18))
      assert.equal((await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString(), dec(1000, 18))
    ...
    it('provideToSP(): providing 0 SLSD reverts', async () => {
      // --- SETUP ---
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Bob provides 0 SLSD to the Stability Pool
    ...
    // --- PSY functionality ---
    it('provideToSP(), new deposit: when SP > 0, triggers PSY reward event - increases the sum G', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, 'ether') },
    ...
    it('provideToSP(), new deposit: depositor does not receive any PSY rewards', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, 'ether') },
      ...
      // Get A, B, C PSY balances before and confirm they're zero
      const A_PSYBalance_Before = await psyToken.balanceOf(A)
      const B_PSYBalance_Before = await psyToken.balanceOf(B)

      assert.equal(A_PSYBalance_Before, '0')
      assert.equal(B_PSYBalance_Before, '0')
      ...
      // Get A, B, C PSY balances after, and confirm they're still zero
      const A_PSYBalance_After = await psyToken.balanceOf(A)
      const B_PSYBalance_After = await psyToken.balanceOf(B)

      assert.equal(A_PSYBalance_After, '0')
      assert.equal(B_PSYBalance_After, '0')
    })

    it('provideToSP(), new deposit after past full withdrawal: depositor does not receive any PSY rewards', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const initialDeposit_A = (await slsdToken.balanceOf(A)).div(toBN(2))
      const initialDeposit_B = (await slsdToken.balanceOf(B)).div(toBN(2))
      // A, B provide to SP
      ...
      // Get A, B, C PSY balances before and confirm they're non-zero
      const A_PSYBalance_Before = await psyToken.balanceOf(A)
      const B_PSYBalance_Before = await psyToken.balanceOf(B)
      assert.isTrue(A_PSYBalance_Before.gt(toBN('0')))
      assert.isTrue(B_PSYBalance_Before.gt(toBN('0')))
      ...
      // Get A, B, C PSY balances after, and confirm they have not changed
      const A_PSYBalance_After = await psyToken.balanceOf(A)
      const B_PSYBalance_After = await psyToken.balanceOf(B)

      assert.isTrue(A_PSYBalance_After.eq(A_PSYBalance_Before))
      assert.isTrue(B_PSYBalance_After.eq(B_PSYBalance_Before))
    })
    ...
    it('provideToSP(), new deposit: depositor does not receive ETH gains', async () => {
      ...
      await slsdToken.transfer(A, dec(200, 18), { from: whale })
      await slsdToken.transfer(B, dec(400, 18), { from: whale })
    ...
    it('provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains', async () => {
      ...
      // Whale transfers SLSD to A, B
      await slsdToken.transfer(A, dec(2000, 18), { from: whale })
      await slsdToken.transfer(B, dec(2000, 18), { from: whale })
    ...
    it('provideToSP(), topup: triggers PSY reward event - increases the sum G', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Expect G has increased from the PSY reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before))
      assert.isTrue(G_AfterERC20.gt(G_BeforeERC20))
    })

    it('provideToSP(), topup: depositor receives PSY rewards', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it("provideToSP(), topup: System's snapshots update", async () => {
      ...
      await openTrove({
        extraSLSDAmount: toBN(dec(400, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: B },
      })
      await openTrove({
        extraSLSDAmount: toBN(dec(600, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      ...
      const slsdD_Balance = toBN(await slsdToken.balanceOf(D)).div(toBN(2))
      await stabilityPool.provideToSP(slsdD_Balance, { from: D })
      await stabilityPoolERC20.provideToSP(slsdD_Balance, { from: D })
    ...
    it('provideToSP(): reverts when amount is zero', async () => {
      ...
      // Whale transfers SLSD to C, D
      await slsdToken.transfer(C, dec(200, 18), { from: whale })
      await slsdToken.transfer(D, dec(200, 18), { from: whale })
