...
contract('HintHelpers', async (accounts) => {
  ...
  /* Open a Trove for each account. SLSD debt is 200 SLSD each, with collateral beginning at 1.5 ether, and rising by 0.01 ether per Trove.
   *  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i.
   */

  // Open Troves in parallel, then withdraw SLSD in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdrawSLSDpromises = activeAccounts.map((account) => withdrawSLSDfromTrove(account))
    await Promise.all(withdrawSLSDpromises)
  ...
  const withdrawSLSDfromTrove = async (account) => {
    await borrowerOperations.withdrawSLSD(th._100pct, '100000000000000000000', account, account, {
      from: account,
    })
  }

  // Sequentially add coll and withdraw SLSD, 1 account at a time
