  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      if (withProxy) {
        ...
        await deploymentHelper.deployProxyScripts(contracts, PSYContracts, owner, users)
      }
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_SLSDDebt snapshots", async () => {
      // --- SETUP ---
      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({
        extraSLSDAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      })
      ...
      // price drops to 1ETH:100SLSD, reducing Carol's ICR below MCR
      ...
      const L_SLSDDebt = await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)
      ...
      assert.equal(alice_SLSDDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ETHrewardSnapshot_Before, 0)
      assert.equal(bob_SLSDDebtRewardSnapshot_Before, 0)

      assert.equal(alice_ETHrewardSnapshot_Before_Asset, 0)
      assert.equal(alice_SLSDDebtRewardSnapshot_Before_Asset, 0)
      assert.equal(bob_ETHrewardSnapshot_Before_Asset, 0)
      assert.equal(bob_SLSDDebtRewardSnapshot_Before_Asset, 0)
      ...
      const alicePendingSLSDDebtReward = await troveManagerHelpers.getPendingSLSDDebtReward(
        ZERO_ADDRESS,
        alice
      )
      const bobPendingSLSDDebtReward = await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, bob)
      ...
      const alicePendingSLSDDebtRewardAsset = await troveManagerHelpers.getPendingSLSDDebtReward(
        erc20.address,
        alice
      )
      const bobPendingSLSDDebtRewardAsset = await troveManagerHelpers.getPendingSLSDDebtReward(
        erc20.address,
        bob
      )
      ...
      for (reward of [
        ...
        bobPendingETHReward,
        alicePendingSLSDDebtReward,
        bobPendingSLSDDebtReward,
        alicePendingETHRewardAsset,
        bobPendingETHRewardAsset,
        alicePendingSLSDDebtRewardAsset,
        bobPendingSLSDDebtRewardAsset,
      ]) {
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingSLSDDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingSLSDDebtReward)))

      assert.isTrue(
        aliceNewColl_Asset.eq(aliceCollBeforeAsset.add(alicePendingETHRewardAsset).add(aliceTopUp))
      )
      assert.isTrue(aliceNewDebt_Asset.eq(aliceDebtBeforeAsset.add(alicePendingSLSDDebtRewardAsset)))
      ...
      assert.isAtMost(th.getDifference(bob_SLSDDebtRewardSnapshot_After, L_SLSDDebt), 100)

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After_Asset, L_Asset), 100)
      assert.isAtMost(th.getDifference(alice_SLSDDebtRewardSnapshot_After_Asset, L_SLSDDebt_Asset), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After_Asset, L_Asset), 100)
      assert.isAtMost(th.getDifference(bob_SLSDDebtRewardSnapshot_After_Asset, L_SLSDDebt_Asset), 100)
    })
    ...
    it('withdrawColl(): reverts if system is in Recovery Mode', async () => {
      ...
      // price drops to 1ETH:150SLSD, reducing TCR below 150%
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_SLSDDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180SLSD.
      await troveManager.liquidate(ZERO_ADDRESS, carol, { from: owner })
      await troveManager.liquidate(erc20.address, carol, { from: owner })
      ...
      const pendingDebtReward_A = await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, alice)
      const pendingCollReward_B = await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob)
      const pendingDebtReward_B = await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, bob)
      ...
      const pendingDebtReward_A_Asset = await troveManagerHelpers.getPendingSLSDDebtReward(
        erc20.address,
        alice
      )
      const pendingCollReward_B_Asset = await troveManagerHelpers.getPendingAssetReward(erc20.address, bob)
      const pendingDebtReward_B_Asset = await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, bob)
      for (reward of [
    ...
    it('withdrawSLSD(): reverts when withdrawal would leave trove with ICR < MCR', async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const SLSDwithdrawal = 1 // withdraw 1 wei SLSD

      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, SLSDwithdrawal, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, th._100pct, SLSDwithdrawal, alice, alice, {
          from: alice,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    ...
    it('withdrawSLSD(): decays a non-zero base rate', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(20, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      const A_SLSDBal = await slsdToken.balanceOf(A)
      ...
      // D withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(1, 18), A, A, { from: D })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(1, 18), A, A, { from: E })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(1, 18), A, A, { from: E })
    ...
    it('withdrawSLSD(): reverts if max fee > 100%', async () => {
      await openTrove({ extraSLSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(40, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(2, 18), dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, '1000000000000000001', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )

      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, dec(2, 18), dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, '1000000000000000001', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    })

    it('withdrawSLSD(): reverts if max fee < 0.5% in Normal mode', async () => {
      await openTrove({ extraSLSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, 0, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, 1, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, '4999999999999999', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )

      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, 0, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, 1, dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, '4999999999999999', dec(1, 18), A, A, { from: A }),
        'Max fee percentage must be between 0.5% and 100%'
      )
    })

    it('withdrawSLSD(): reverts if fee exceeds max fee percentage', async () => {
      await openTrove({ extraSLSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraSLSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        extraSLSDAmount: toBN(dec(180, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      })
      ...
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(70, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(80, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
      await openTrove({
      ...
      const totalSupply = await slsdToken.totalSupply()
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, lessThan5pct, dec(3, 18), A, A, { from: A }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, lessThan5pct, dec(3, 18), A, A, { from: A }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(1, 16), dec(1, 18), A, A, { from: B }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, dec(1, 16), dec(1, 18), A, A, { from: B }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(3754, 13), dec(1, 18), A, A, { from: C }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, dec(3754, 13), dec(1, 18), A, A, { from: C }),
        'Fee exceeded provided maximum'
      )
      ...
      await assertRevert(
        borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(5, 15), dec(1, 18), A, A, { from: D }),
        'Fee exceeded provided maximum'
      )
      await assertRevert(
        borrowerOperations.withdrawSLSD(erc20.address, dec(5, 15), dec(1, 18), A, A, { from: D }),
        'Fee exceeded provided maximum'
      )
    })

    it('withdrawSLSD(): succeeds when fee is less than max fee percentage', async () => {
      await openTrove({ extraSLSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraSLSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, moreThan5pct, dec(1, 18), A, A, {
        from: A,
      })
      const tx1_Asset = await borrowerOperations.withdrawSLSD(erc20.address, moreThan5pct, dec(1, 18), A, A, {
        from: A,
      })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(5, 16), dec(1, 18), A, A, {
        from: B,
      })
      const tx2_Asset = await borrowerOperations.withdrawSLSD(erc20.address, dec(5, 16), dec(1, 18), A, A, {
        from: B,
      })
      ...
      const tx3 = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(1, 17), dec(1, 18), A, A, {
        from: C,
      })
      const tx3_Asset = await borrowerOperations.withdrawSLSD(erc20.address, dec(1, 17), dec(1, 18), A, A, {
        from: C,
      })
      ...
      const tx4 = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(37659, 13), dec(1, 18), A, A, {
        from: D,
      })
      const tx4_Asset = await borrowerOperations.withdrawSLSD(
        erc20.address,
        dec(37659, 13),
        dec(1, 18),
        A,
      ...
      const tx5 = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, dec(1, 18), dec(1, 18), A, A, {
        from: E,
      })
      const tx5_Asset = await borrowerOperations.withdrawSLSD(erc20.address, dec(1, 18), dec(1, 18), A, A, {
        from: E,
      })
    ...
    it("withdrawSLSD(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraSLSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await openTrove({
      ...
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(50, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
      await openTrove({
      ...
      // D withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(37, 18), A, A, { from: D })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(12, 18), A, A, { from: E })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(12, 18), A, A, { from: E })
    ...
    it("withdrawSLSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(1, 18), C, C, { from: C })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdrawSLSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraSLSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it('withdrawSLSD(): borrowing at non-zero base rate sends SLSD fee to PSY staking contract', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })
      ...
      // D withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(37, 18), C, C, { from: D })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(37, 18), C, C, { from: D })
    ...
    if (!withProxy) {
      ...
      it('withdrawSLSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct', async () => {
        // time fast-forwards 1 year, and multisig stakes 1 PSY
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
        await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
        await psyStaking.stake(dec(1, 18), { from: multisig })
        ...
        const withdrawalTx = await borrowerOperations.withdrawSLSD(
          ZERO_ADDRESS,
          th._100pct,
          toBN(dec(37, 18)),
          D,
        ...
        const withdrawalTx_Asset = await borrowerOperations.withdrawSLSD(
          erc20.address,
          th._100pct,
          toBN(dec(37, 18)),
          D,
        ...
        const emittedFee = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(withdrawalTx))
        const emittedFee_Asset = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(withdrawalTx_Asset))
    ...
    it('withdrawSLSD(): Borrowing at non-zero base rate increases the PSY staking contract SLSD fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })

      // Check PSY contract SLSD fees-per-unit-staked is zero
      const F_SLSD_Before = await psyStaking.F_SLSD()
      assert.equal(F_SLSD_Before, '0')
      ...
      // D withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, toBN(dec(37, 18)), D, D, { from: D })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, toBN(dec(37, 18)), D, D, { from: D })

      // Check PSY contract SLSD fees-per-unit-staked has increased
      const F_SLSD_After = await psyStaking.F_SLSD()
      assert.isTrue(F_SLSD_After.gt(F_SLSD_Before))
    })

    it('withdrawSLSD(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })

      // Check PSY Staking contract balance before == 0
      const PSYStaking_SLSDBalance_Before = await slsdToken.balanceOf(psyStaking.address)
      assert.equal(PSYStaking_SLSDBalance_Before, '0')
      ...
      let D_SLSDBalanceBefore = await slsdToken.balanceOf(D)

      // D withdraws SLSD
      const D_SLSDRequest = toBN(dec(37, 18))
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, D_SLSDRequest, D, D, { from: D })

      // Check PSY staking SLSD balance has increased
      let PSYStaking_SLSDBalance_After = await slsdToken.balanceOf(psyStaking.address)
      assert.isTrue(PSYStaking_SLSDBalance_After.gt(PSYStaking_SLSDBalance_Before))

      // Check D's SLSD balance now equals their initial balance plus request SLSD
      let D_SLSDBalanceAfter = await slsdToken.balanceOf(D)
      assert.isTrue(D_SLSDBalanceAfter.eq(D_SLSDBalanceBefore.add(D_SLSDRequest)))

      //Asset:
      D_SLSDBalanceBefore = await slsdToken.balanceOf(D)
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, D_SLSDRequest, D, D, { from: D })

      PSYStaking_SLSDBalance_After = await slsdToken.balanceOf(psyStaking.address)
      assert.isTrue(PSYStaking_SLSDBalance_After.gt(PSYStaking_SLSDBalance_Before))

      D_SLSDBalanceAfter = await slsdToken.balanceOf(D)
      assert.isTrue(D_SLSDBalanceAfter.eq(D_SLSDBalanceBefore.add(D_SLSDRequest)))
    })

    it('withdrawSLSD(): Borrowing at zero base rate changes SLSD fees-per-unit-staked', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraSLSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // A artificially receives PSY, then stakes it
      await psyToken.unprotectedMint(A, dec(100, 18))
      ...
      // D withdraws SLSD
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(37, 18), D, D, { from: D })

      // Check PSY SLSD balance after > 0
      const F_SLSD_After = await psyStaking.F_SLSD()
      assert.isTrue(F_SLSD_After.gt('0'))
    })

    it('withdrawSLSD(): Borrowing at zero base rate sends debt request to user', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraSLSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      let D_SLSDBalanceAfter = await slsdToken.balanceOf(D)
      // Check D's trove debt == D's SLSD balance + liquidation reserve
      assert.isTrue(D_SLSDBalanceAfter.eq(D_SLSDBalanceBefore.add(D_SLSDRequest)))

      D_SLSDBalanceBefore = await slsdToken.balanceOf(D)
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(37, 18), D, D, { from: D })

      D_SLSDBalanceAfter = await slsdToken.balanceOf(D)
      assert.isTrue(D_SLSDBalanceAfter.eq(D_SLSDBalanceBefore.add(D_SLSDRequest)))
    })

    it('withdrawSLSD(): reverts when calling address does not have active trove', async () => {
      ...
      // Bob successfully withdraws SLSD
      const txBob = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(100, 18), bob, bob, {
        from: bob,
      })
      const txBob_Asset = await borrowerOperations.withdrawSLSD(
        erc20.address,
        th._100pct,
        dec(100, 18),
        bob,
      ...
      // Carol with no active trove attempts to withdraw SLSD
      try {
        const txCarol = await borrowerOperations.withdrawSLSD(
          ZERO_ADDRESS,
          th._100pct,
          dec(100, 18),
          carol,
      ...
      try {
        const txCarol_Asset = await borrowerOperations.withdrawSLSD(
          erc20.address,
          th._100pct,
          dec(100, 18),
          carol,
    ...
    it('withdrawSLSD(): reverts when requested withdrawal amount is zero SLSD', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18 SLSD
      const txBob = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
        from: bob,
      })
      const txBob_Asset = await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, 1, bob, bob, {
        from: bob,
      })
      assert.isTrue(txBob.receipt.status)
      assert.isTrue(txBob_Asset.receipt.status)

      // Alice attempts to withdraw 0 SLSD
      try {
        const txAlice = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, 0, alice, alice, {
          from: alice,
        })
      ...
      try {
        const txAlice_Asset = await borrowerOperations.withdrawSLSD(
          erc20.address,
          th._100pct,
          0,
          alice,
    ...
    it('withdrawSLSD(): reverts when system is in Recovery Mode', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawSLSD(
        ZERO_ADDRESS,
        th._100pct,
        dec(100, 18),
        alice,
      ...
      //Check SLSD withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
          from: bob,
        })
    ...
    it("withdrawSLSD(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      // Bob tries to withdraw SLSD that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, 1, bob, bob, {
          from: bob,
        })
    ...
    it('withdrawSLSD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR', async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1 SLSD.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(1, 18), bob, bob, {
          from: bob,
        })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it('withdrawSLSD(): reverts if system is in Recovery Mode', async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, '200', alice, alice, {
          from: alice,
        })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
      ...
      try {
        const txData = await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, '200', alice, alice, {
          from: alice,
        })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
    ...
    it("withdrawSLSD(): increases the Trove's SLSD debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ asset: erc20.address, ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdrawSLSD(
        ZERO_ADDRESS,
        th._100pct,
    ...
    it('withdrawSLSD(): increases SLSD debt in ActivePool by correct amount', async () => {
      await openTrove({
        ICR: toBN(dec(10, 18)),
      ...
      // check before
      const activePool_SLSD_Before = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePool_SLSD_Before_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.isTrue(activePool_SLSD_Before.eq(aliceDebtBefore))
      assert.isTrue(activePool_SLSD_Before_Asset.eq(aliceDebtBefore_Asset))
      ...
      // check after
      const activePool_SLSD_After = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePool_SLSD_After_Asset = await activePool.getSLSDDebt(erc20.address)
      th.assertIsApproximatelyEqual(activePool_SLSD_After, activePool_SLSD_Before.add(toBN(dec(10000, 18))))
      th.assertIsApproximatelyEqual(
        activePool_SLSD_After_Asset,
        activePool_SLSD_Before_Asset.add(toBN(dec(10000, 18)))
      )
    })

    it('withdrawSLSD(): increases user SLSDToken balance by correct amount', async () => {
      ...
      // check before
      let alice_SLSDTokenBalance_Before = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_Before.gt(toBN('0')))

      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
      })

      let alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_After.eq(alice_SLSDTokenBalance_Before.add(toBN(dec(10000, 18)))))
      alice_SLSDTokenBalance_Before = await slsdToken.balanceOf(alice)

      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(10000, 18), alice, alice, {
        from: alice,
      })

      alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_After.eq(alice_SLSDTokenBalance_Before.add(toBN(dec(10000, 18)))))
    })

    // --- repaySLSD() ---
    it('repaySLSD(): reverts when repayment would leave trove with ICR < MCR', async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const SLSDRepayment = 1 // 1 wei repayment

      await assertRevert(
        borrowerOperations.repaySLSD(ZERO_ADDRESS, SLSDRepayment, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
      await assertRevert(
        borrowerOperations.repaySLSD(erc20.address, SLSDRepayment, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    })

    it('repaySLSD(): Succeeds when it would leave trove with net debt >= minimum net debt', async () => {
      // Make the SLSD request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      ...
      const repayTxA = await borrowerOperations.repaySLSD(ZERO_ADDRESS, 1, A, A, { from: A })
      const repayTxA_Asset = await borrowerOperations.repaySLSD(erc20.address, 1, A, A, { from: A })
      ...
      const repayTxB = await borrowerOperations.repaySLSD(ZERO_ADDRESS, dec(19, 25), B, B, { from: B })
      const repayTxB_Asset = await borrowerOperations.repaySLSD(erc20.address, dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
      assert.isTrue(repayTxB_Asset.receipt.status)
    })

    it('repaySLSD(): reverts when it would leave trove with net debt < minimum net debt', async () => {
      // Make the SLSD request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      const repayTxAPromise = borrowerOperations.repaySLSD(ZERO_ADDRESS, 2, A, A, { from: A })
      const repayTxAPromise_Asset = borrowerOperations.repaySLSD(erc20.address, 2, A, A, { from: A })
    ...
    it('repaySLSD(): reverts when calling address does not have active trove', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      // Bob successfully repays some SLSD
      const txBob = await borrowerOperations.repaySLSD(ZERO_ADDRESS, dec(10, 18), bob, bob, { from: bob })
      const txBob_Asset = await borrowerOperations.repaySLSD(erc20.address, dec(10, 18), bob, bob, {
        from: bob,
      })
      assert.isTrue(txBob.receipt.status)
      assert.isTrue(txBob_Asset.receipt.status)

      // Carol with no active trove attempts to withdrawSLSD
      try {
        const txCarol = await borrowerOperations.repaySLSD(ZERO_ADDRESS, dec(10, 18), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
      ...
      try {
        const txCarol_Asset = await borrowerOperations.repaySLSD(erc20.address, dec(10, 18), carol, carol, {
          from: carol,
        })
        assert.isFalse(txCarol_Asset.receipt.status)
      } catch (err) {
    ...
    it('repaySLSD(): reverts when attempted repayment is > the debt of the trove', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      try {
        const txAlice = await borrowerOperations.repaySLSD(
          ZERO_ADDRESS,
      ...
      try {
        const txAlice_Asset = await borrowerOperations.repaySLSD(
          erc20.address,
    ...
    //withdrawSLSD: reduces SLSD debt in Trove
    it("repaySLSD(): reduces the Trove's SLSD debt by the correct amount", async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      await borrowerOperations.repaySLSD(ZERO_ADDRESS, aliceDebtBefore.div(toBN(10)), alice, alice, {
        from: alice,
      }) // Repays 1/10 her debt
      await borrowerOperations.repaySLSD(erc20.address, aliceDebtBefore_Asset.div(toBN(10)), alice, alice, {
        from: alice,
      }) // Repays 1/10 her debt
    ...
    it('repaySLSD(): decreases SLSD debt in ActivePool by correct amount', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const activePool_SLSD_Before = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePool_SLSD_Before_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.isTrue(activePool_SLSD_Before.gt(toBN('0')))
      assert.isTrue(activePool_SLSD_Before_Asset.gt(toBN('0')))
      ...
      const activePool_SLSD_After_Asset = await activePool.getSLSDDebt(erc20.address)
      th.assertIsApproximatelyEqual(
        activePool_SLSD_After,
        activePool_SLSD_Before.sub(aliceDebtBefore.div(toBN(10)))
      )
      th.assertIsApproximatelyEqual(
        activePool_SLSD_After_Asset,
        activePool_SLSD_Before_Asset.sub(aliceDebtBefore_Asset.div(toBN(10)))
      )
    })

    it('repaySLSD(): decreases user SLSDToken balance by correct amount', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      let alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(
        alice_SLSDTokenBalance_After,
        alice_SLSDTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10)))
      )
      alice_SLSDTokenBalance_Before = await slsdToken.balanceOf(alice)
      ...
      th.assertIsApproximatelyEqual(
        alice_SLSDTokenBalance_After,
        alice_SLSDTokenBalance_Before.sub(aliceDebtBefore_Asset.div(toBN(10)))
      )
    })

    it('repaySLSD(): can repay debt in Recovery Mode', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const tx = await borrowerOperations.repaySLSD(
        ZERO_ADDRESS,
      ...
      const tx_Asset = await borrowerOperations.repaySLSD(
        erc20.address,
    ...
    it('repaySLSD(): Reverts if borrower has insufficient SLSD balance to cover his debt repayment', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      ...
      const bobBalBefore = await slsdToken.balanceOf(B)
      assert.isTrue(bobBalBefore.gt(toBN('0')))

      // Bob transfers all but 5 of his SLSD to Carol
      await slsdToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), { from: B })

      //Confirm B's SLSD balance has decreased to 5 SLSD
      const bobBalAfter = await slsdToken.balanceOf(B)

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))

      // Bob tries to repay 6 SLSD
      const repaySLSDPromise_B = borrowerOperations.repaySLSD(ZERO_ADDRESS, toBN(dec(6, 18)), B, B, {
        from: B,
      })
      const repaySLSDPromise_B_Asset = borrowerOperations.repaySLSD(erc20.address, toBN(dec(6, 18)), B, B, {
        from: B,
      })

      await assertRevert(repaySLSDPromise_B, 'Caller doesnt have enough SLSD to make repayment')
      await assertRevert(repaySLSDPromise_B_Asset, 'Caller doesnt have enough SLSD to make repayment')
    })
    ...
    it('adjustTrove(): reverts when adjustment would leave trove with ICR < MCR', async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, SLSDRepayment, false, alice, alice, {
          from: alice,
          value: collTopUp,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, SLSDRepayment, false, alice, alice, {
          from: alice,
          value: collTopUp,
        }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it('adjustTrove(): borrowing at non-zero base rate sends SLSD fee to PSY staking contract', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })
      ...
      // D adjusts trove
      await openTrove({ extraSLSDAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(37, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
    ...
    if (!withProxy) {
      ...
      it('adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct', async () => {
        ...
        const emittedFee = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))

        const emittedFee_Asset = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(adjustmentTx_Asset))
        assert.isTrue(emittedFee_Asset.gt(toBN('0')))
    ...
    it('adjustTrove(): Borrowing at non-zero base rate increases the PSY staking contract SLSD fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })
      ...
      const F_SLSD_After_Asset = await psyStaking.F_SLSD()
      assert.isTrue(F_SLSD_After_Asset.gt(F_SLSD_After))
    })
    ...
    it('adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      ...
      const D_SLSDBalanceBefore = await slsdToken.balanceOf(D)
      ...
      // D adjusts trove
      const SLSDRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, SLSDRequest_D, true, D, D, {
        from: D,
      })
      ...
      // Check D's SLSD balance has increased by their requested SLSD
      const D_SLSDBalanceAfter = await slsdToken.balanceOf(D)
      assert.isTrue(D_SLSDBalanceAfter.eq(D_SLSDBalanceBefore.add(SLSDRequest_D)))

      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, SLSDRequest_D, true, D, D, {
        from: D,
      })

      // Check PSY staking SLSD balance has increased
      const PSYStaking_SLSDBalance_After_Asset = await slsdToken.balanceOf(psyStaking.address)
      assert.isTrue(PSYStaking_SLSDBalance_After_Asset.gt(PSYStaking_SLSDBalance_After))

      // Check D's SLSD balance has increased by their requested SLSD
      const D_SLSDBalanceAfter_Asset = await slsdToken.balanceOf(D)
      assert.isTrue(D_SLSDBalanceAfter_Asset.eq(D_SLSDBalanceAfter.add(SLSDRequest_D)))
    })

    it('adjustTrove(): Borrowing at zero base rate changes SLSD balance of PSY staking contract', async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraSLSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraSLSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraSLSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check staking SLSD balance before > 0
      const PSYStaking_SLSDBalance_Before = await slsdToken.balanceOf(psyStaking.address)
      assert.isTrue(PSYStaking_SLSDBalance_Before.gt(toBN('0')))
    ...
    it('adjustTrove(): Borrowing at zero base rate changes PSY staking contract SLSD fees-per-unit-staked', async () => {
      await openTrove({
      ...
      // Check staking SLSD balance before == 0
      const F_SLSD_Before = await psyStaking.F_SLSD()
      assert.isTrue(F_SLSD_Before.eq(toBN('0')))
    ...
    it('adjustTrove(): Borrowing at zero base rate sends total requested SLSD to the user', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: toBN(dec(100, 'ether')) },
      ...
      const D_SLSDBalBefore = await slsdToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManagerHelpers.baseRate(ZERO_ADDRESS)
      assert.equal(baseRate_1, '0')
      ...
      const SLSDBalanceAfter = await slsdToken.balanceOf(D)
      assert.isTrue(SLSDBalanceAfter.eq(D_SLSDBalBefore.add(SLSDRequest_D)))

      await borrowerOperations.adjustTrove(erc20.address, 0, th._100pct, 0, SLSDRequest_D, true, D, D, {
        from: D,
      })
      const SLSDBalanceAfter_Asset = await slsdToken.balanceOf(D)
      assert.isTrue(SLSDBalanceAfter_Asset.eq(SLSDBalanceAfter.add(SLSDRequest_D)))
    })
    ...
    it('adjustTrove(): reverts when calling address has no active trove', async () => {
      ...
      // Alice coll and debt increase(+1 ETH, +50SLSD)
    ...
    it('adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR But collateral is blocked from minting, then unblock it', async () => {
      ...
      await contracts.slsdToken.emergencyStopMinting(erc20.address, true)
      await contracts.slsdToken.emergencyStopMinting(ZERO_ADDRESS, true)
      ...
      await contracts.slsdToken.emergencyStopMinting(erc20.address, false)
      await contracts.slsdToken.emergencyStopMinting(ZERO_ADDRESS, false)
    ...
    it('adjustTrove(): debt increase in Recovery Mode charges no fee', async () => {
      ...
      await openTrove({
        extraSLSDAmount: toBN(dec(200000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: bob },
      })
      ...
      // B stakes PSY
      await psyToken.unprotectedMint(bob, dec(100, 18))
      await psyStaking.stake(dec(100, 18), { from: bob })

      const PSYStakingSLSDBalanceBefore = await slsdToken.balanceOf(psyStaking.address)
      assert.isTrue(PSYStakingSLSDBalanceBefore.gt(toBN('0')))
      ...
      const PSYStakingSLSDBalanceAfter = await slsdToken.balanceOf(psyStaking.address)
      assert.equal(PSYStakingSLSDBalanceAfter.toString(), PSYStakingSLSDBalanceBefore.toString())
    })
    ...
    it('adjustTrove(): reverts when SLSD repaid is > debt of the trove', async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'SLSDBorrowingFeePaid', 2))
      assert.isTrue(bobFee.gt(toBN('0')))

      const bobFee_Asset = toBN(await th.getEventArgByIndex(bobOpenTx_Asset, 'SLSDBorrowingFeePaid', 2))
      assert.isTrue(bobFee_Asset.gt(toBN('0')))

      // Alice transfers SLSD to bob to compensate borrowing fees
      await slsdToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManagerHelpers.getTroveDebt(ZERO_ADDRESS, bob)).sub(
        SLSD_GAS_COMPENSATION
      )
    ...
    it('adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR', async () => {
      ...
      // Bob attempts to increase debt by 100 SLSD and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50SLSD)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: 0,
      })
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.getSLSDDebt(ZERO_ADDRESS)

      const aliceDebtBefore_Asset = await getTroveEntireDebt(alice, erc20.address)
      const activePoolDebtBefore_Asset = await activePool.getSLSDDebt(erc20.address)
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.getSLSDDebt(ZERO_ADDRESS)

      const aliceDebtAfter_Asset = await getTroveEntireDebt(alice, erc20.address)
      const activePoolDebtAfter_Asset = await activePool.getSLSDDebt(erc20.address)
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50SLSD)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50SLSD)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50SLSD)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10SLSD)
      await borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 SLSD)
      await borrowerOperations.adjustTrove(ZERO_ADDRESS, 0, th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: dec(1, 'ether'),
      })
    ...
    it('adjustTrove(): changes SLSDToken balance by the requested decrease', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const alice_SLSDTokenBalance_Before = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_Before.gt(toBN('0')))
      ...
      // check after
      const alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_After.eq(alice_SLSDTokenBalance_Before.sub(toBN(dec(10, 18)))))
      ...
      // check after
      const alice_SLSDTokenBalance_After_Asset = await slsdToken.balanceOf(alice)
      assert.isTrue(
        alice_SLSDTokenBalance_After_Asset.eq(alice_SLSDTokenBalance_After.sub(toBN(dec(10, 18))))
      )
    })

    it('adjustTrove(): changes SLSDToken balance by the requested increase', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // check after
      const alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDTokenBalance_After.eq(alice_SLSDTokenBalance_Before.add(toBN(dec(100, 18)))))
      ...
      const alice_SLSDTokenBalance_After_Asset = await slsdToken.balanceOf(alice)
      assert.isTrue(
        alice_SLSDTokenBalance_After_Asset.eq(alice_SLSDTokenBalance_After.add(toBN(dec(100, 18))))
      )
    })
    ...
    it('adjustTrove(): Changes the SLSD debt in ActivePool by requested decrease', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const activePooL_SLSDDebt_Before = await activePool.getSLSDDebt(ZERO_ADDRESS)
      assert.isTrue(activePooL_SLSDDebt_Before.gt(toBN('0')))

      const activePooL_SLSDDebt_Before_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.isTrue(activePooL_SLSDDebt_Before_Asset.gt(toBN('0')))
      ...
      const activePooL_SLSDDebt_After = await activePool.getSLSDDebt(ZERO_ADDRESS)
      assert.isTrue(activePooL_SLSDDebt_After.eq(activePooL_SLSDDebt_Before.sub(toBN(dec(30, 18)))))

      const activePooL_SLSDDebt_After_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.isTrue(
        activePooL_SLSDDebt_After_Asset.eq(activePooL_SLSDDebt_Before_Asset.sub(toBN(dec(30, 18))))
      )
    })

    it('adjustTrove(): Changes the SLSD debt in ActivePool by requested increase', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      const activePooL_SLSDDebt_After = await activePool.getSLSDDebt(ZERO_ADDRESS)
      th.assertIsApproximatelyEqual(
        activePooL_SLSDDebt_After,
        activePooL_SLSDDebt_Before.add(toBN(dec(100, 18)))
      )

      const activePooL_SLSDDebt_After_Asset = await activePool.getSLSDDebt(erc20.address)
      th.assertIsApproximatelyEqual(
        activePooL_SLSDDebt_After_Asset,
        activePooL_SLSDDebt_Before_Asset.add(toBN(dec(100, 18)))
      )
    })
    ...
    it('adjustTrove(): Reverts if borrower has insufficient SLSD balance to cover his debt repayment', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Bob transfers some SLSD to carol
      await slsdToken.transfer(C, dec(10, 18), { from: B })

      //Confirm B's SLSD balance is less than 50 SLSD
      const B_SLSDBal = await slsdToken.balanceOf(B)
      assert.isTrue(B_SLSDBal.lt(bobDebt))

      const repaySLSDPromise_B = borrowerOperations.adjustTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        0,
      ...
      await assertRevert(repaySLSDPromise_B, 'revert')

      await slsdToken.transfer(C, B_SLSDBal, { from: B })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      const B_SLSDBal_Asset = await slsdToken.balanceOf(B)
      assert.isTrue(B_SLSDBal_Asset.lt(bobDebt_Asset))

      const repaySLSDPromise_B_Asset = borrowerOperations.adjustTrove(
        erc20.address,
        0,
        th._100pct,
        0,
      ...
      await assertRevert(repaySLSDPromise_B_Asset, 'revert')
    })
    ...
    it('closeTrove(): reverts when it would lower the TCR below CCR', async () => {
      ...
      await openTrove({
        ICR: toBN(dec(120, 16)),
        extraSLSDAmount: toBN(dec(300, 18)),
        extraParams: { from: bob },
      })
      ...
      await slsdToken.transfer(alice, dec(300, 18), { from: bob })
    ...
    it('closeTrove(): reverts when system is in Recovery Mode', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(100000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice },
      })
      await openTrove({
      ...
      // Alice transfers her SLSD to Bob and Carol so they can cover fees
      const aliceBal = await slsdToken.balanceOf(alice)
      await slsdToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice })
      await slsdToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice })
    ...
    it('closeTrove(): reverts when trove is the only one in the system', async () => {
      ...
      await slsdToken.unprotectedMint(alice, dec(100000, 18))

      // Check she has more SLSD than her trove debt
      const aliceBal = await slsdToken.balanceOf(alice)
      const aliceDebt = await getTroveEntireDebt(alice)
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisSLSD = await slsdToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisSLSD.gt(toBN('0')))

      const aliceCollBefore_Asset = await getTroveEntireColl(alice, erc20.address)
      assert.isTrue(aliceCollBefore_Asset.gt(toBN('0')))

      // To compensate borrowing fees
      await slsdToken.transfer(alice, dennisSLSD.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      assert.isTrue(L_SLSDDebt_Snapshot_A_AfterLiquidation.gt(toBN('0')))

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation_Asset.gt(toBN('0')))
      assert.isTrue(L_SLSDDebt_Snapshot_A_AfterLiquidation_Asset.gt(toBN('0')))

      // to compensate borrowing fees
      await slsdToken.transfer(alice, await slsdToken.balanceOf(dennis), { from: dennis })
      ...
      assert.equal(L_SLSDDebt_Snapshot_A_afterAliceCloses_Asset, '0')
    })
    ...
    it('closeTrove(): reduces ActivePool debt by correct amount', async () => {
      ...
      // Check before
      const activePool_Debt_before = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePool_Debt_before_Asset = await activePool.getSLSDDebt(erc20.address)
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getSLSDDebt(ZERO_ADDRESS)).toString()
      th.assertIsApproximatelyEqual(activePool_Debt_After, dennisDebt)
      const activePool_Debt_After_Asset = (await activePool.getSLSDDebt(erc20.address)).toString()
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's SLSDToken balance", async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: dennis },
      ...
      const alice_SLSDBalance_Before = await slsdToken.balanceOf(alice)
      assert.isTrue(alice_SLSDBalance_Before.gt(toBN('0')))

      // close trove
      await borrowerOperations.closeTrove(ZERO_ADDRESS, { from: alice })
      await borrowerOperations.closeTrove(erc20.address, { from: alice })

      // check alice SLSD balance after
      const alice_SLSDBalance_After = await slsdToken.balanceOf(alice)
      const debtValue = aliceDebt.sub(SLSD_GAS_COMPENSATION)
      const debtValueERC20 = aliceDebt.sub(SLSD_GAS_COMPENSATION_ERC20)
      th.assertIsApproximatelyEqual(
        alice_SLSDBalance_After,
        alice_SLSDBalance_Before.sub(debtValue.add(debtValueERC20))
      )
    })
    ...
    it('closeTrove(): applies pending rewards', async () => {
      // --- SETUP ---
      await openTrove({
        extraSLSDAmount: toBN(dec(1000000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      })
      await openTrove({
      ...
      await slsdToken.transfer(alice, dec(10000, 18), { from: whale })
      await slsdToken.transfer(bob, dec(10000, 18), { from: whale })
      ...
      const defaultPooL_SLSDDebt = await defaultPool.getSLSDDebt(ZERO_ADDRESS)

      const defaultPool_ETH_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_SLSDDebt_Asset = await defaultPool.getSLSDDebt(erc20.address)

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPooL_SLSDDebt, liquidatedDebt_C), 100)

      assert.isAtMost(th.getDifference(defaultPool_ETH_Asset, liquidatedColl_C_Asset), 100)
      assert.isAtMost(th.getDifference(defaultPooL_SLSDDebt_Asset, liquidatedDebt_C_Asset), 100)
      ...
      const defaultPooL_SLSDDebt_afterAliceCloses = await defaultPool.getSLSDDebt(ZERO_ADDRESS)

      const defaultPool_ETH_afterAliceCloses_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_SLSDDebt_afterAliceCloses_Asset = await defaultPool.getSLSDDebt(erc20.address)
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPooL_SLSDDebt_afterAliceCloses,
          defaultPooL_SLSDDebt.sub(pendingDebtReward_A)
        ),
        1000
      )
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPooL_SLSDDebt_afterAliceCloses_Asset,
          defaultPooL_SLSDDebt.sub(pendingDebtReward_A_Asset)
        ),
        1000
      )
      ...
      const defaultPooL_SLSDDebt_afterBobCloses = await defaultPool.getSLSDDebt(ZERO_ADDRESS)

      const defaultPool_ETH_afterBobCloses_Asset = await defaultPool.getAssetBalance(erc20.address)
      const defaultPooL_SLSDDebt_afterBobCloses_Asset = await defaultPool.getSLSDDebt(erc20.address)

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPooL_SLSDDebt_afterBobCloses, 0), 100000)

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses_Asset, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPooL_SLSDDebt_afterBobCloses_Asset, 0), 100000)
    })

    it('closeTrove(): reverts if borrower has insufficient SLSD balance to repay his entire debt', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      //Confirm Bob's SLSD balance is less than his trove debt
      const B_SLSDBal = await slsdToken.balanceOf(B)
      const B_troveDebt = await getTroveEntireDebt(B)

      assert.isTrue(B_SLSDBal.lt(B_troveDebt))

      const closeTrovePromise_B = borrowerOperations.closeTrove(ZERO_ADDRESS, { from: B })
      await assertRevert(closeTrovePromise_B, 'BorrowerOps: Caller doesnt have enough SLSD to make repayment')

      await slsdToken.transfer(C, await B_SLSDBal, { from: B })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      assert.isTrue(B_SLSDBal_Asset.lt(B_troveDebt_Asset))
    ...
    if (!withProxy) {
      ...
      it('openTrove(): emits a TroveUpdated event with the correct collateral and debt', async () => {
        ...
        const txC = (
          await openTrove({
            extraSLSDAmount: toBN(dec(3000, 18)),
            ICR: toBN(dec(2, 18)),
            extraParams: { from: C },
          })
        ).tx
    ...
    it('openTrove(): decays a non-zero base rate', async () => {
      ...
      // E opens trove
      await openTrove({ extraSLSDAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(12, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E },
      })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraSLSDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(1, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
      ...
      await openTrove({ extraSLSDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({
        asset: erc20.address,
        extraSLSDAmount: toBN(dec(1, 18)),
        ICR: toBN(dec(2, 18)),
    ...
    it('openTrove(): borrowing at non-zero base rate sends SLSD fee to PSY staking contract', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })
    ...
    if (!withProxy) {
      ...
      it('openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct', async () => {
        ...
        const D_SLSDRequest = toBN(dec(20000, 18))
        ...
        const emittedFee = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const emittedFee_Asset = toBN(th.getSLSDFeeFromSLSDBorrowingEvent(openTroveTx_Asset))
        ...
        th.assertIsApproximatelyEqual(
          newDebt,
          D_SLSDRequest.add(emittedFee).add(SLSD_GAS_COMPENSATION),
          100000
        )
        th.assertIsApproximatelyEqual(
          newDebt_Asset,
          D_SLSDRequest.add(emittedFee_Asset).add(SLSD_GAS_COMPENSATION_ERC20),
          100000
        )
      })
    }

    it('openTrove(): Borrowing at non-zero base rate increases the PSY staking contract SLSD fees-per-unit-staked', async () => {
      // time fast-forwards 1 year, and multisig stakes 1 PSY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await psyToken.approve(psyStaking.address, dec(1, 18), { from: multisig })
      await psyStaking.stake(dec(1, 18), { from: multisig })
    ...
    it('openTrove(): Borrowing at non-zero base rate sends requested amount to the user', async () => {
      ...
      // D opens trove
      const SLSDRequest_D = toBN(dec(40000, 18))
      await borrowerOperations.openTrove(ZERO_ADDRESS, 0, th._100pct, SLSDRequest_D, D, D, {
        from: D,
        value: dec(500, 'ether'),
      })
      await borrowerOperations.openTrove(erc20.address, dec(500, 'ether'), th._100pct, SLSDRequest_D, D, D, {
        from: D,
      })
      ...
      // Check D's SLSD balance now equals their requested SLSD
      const SLSDBalance_D = await slsdToken.balanceOf(D)
      assert.isTrue(SLSDRequest_D.mul(toBN(2)).eq(SLSDBalance_D))
    })

    it('openTrove(): Borrowing at zero base rate changes the PSY staking contract SLSD fees-per-unit-staked', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      ...
      // Check SLSD reward per PSY staked == 0
      const F_SLSD_Before = await psyStaking.F_SLSD()
      assert.equal(F_SLSD_Before, '0')
      ...
      // Check SLSD reward per PSY staked > 0
      const F_SLSD_After = await psyStaking.F_SLSD()
      assert.isTrue(F_SLSD_After.gt(toBN('0')))
    })
    ...
    it('openTrove(): Borrowing at zero base rate charges minimum fee', async () => {
      ...
      const SLSDRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      const _SLSDFee_Asset = toBN(th.getEventArgByName(txC_Asset, 'SLSDBorrowingFeePaid', '_SLSDFee'))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(SLSDRequest)).div(toBN(dec(1, 18)))
      const expectedFee_Asset = BORROWING_FEE_FLOOR_ERC20.mul(toBN(SLSDRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_SLSDFee.eq(expectedFee))
      assert.isTrue(_SLSDFee_Asset.eq(expectedFee_Asset))
    })
    ...
    it('openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode', async () => {
      ...
      // price drops to 1ETH:100SLSD, reducing TCR below 150%
      await priceFeed.setPrice('100000000000000000000')
      const price = await priceFeed.getPrice()
    ...
    it('openTrove(): creates a new Trove and assigns the correct collateral and debt amount', async () => {
      ...
      const SLSDRequest = MIN_NET_DEBT
      const SLSDRequestERC20 = MIN_NET_DEBT_ERC20
      ...
      // Get the expected debt based on the SLSD request (adding fee and liq. reserve on top)
      const expectedDebt = SLSDRequest.add(
        await troveManagerHelpers.getBorrowingFee(ZERO_ADDRESS, SLSDRequest)
      ).add(SLSD_GAS_COMPENSATION)

      const expectedDebt_Asset = SLSDRequestERC20.add(
        await troveManagerHelpers.getBorrowingFee(erc20.address, SLSDRequestERC20)
      ).add(SLSD_GAS_COMPENSATION_ERC20)

      await getTroveEntireDebt(alice)
    ...
    it('openTrove(): records up-to-date initial snapshots of L_ETH and L_SLSDDebt', async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_SLSD should equal 18 SLSD per-ether-staked. */

      const L_ETH = await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)
      const L_SLSD = await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)

      const L_Asset = await troveManagerHelpers.L_ASSETS(erc20.address)
      const L_SLSD_Asset = await troveManagerHelpers.L_SLSDDebts(erc20.address)

      assert.isTrue(L_ETH.gt(toBN('0')))
      assert.isTrue(L_SLSD.gt(toBN('0')))
      assert.isTrue(L_Asset.gt(toBN('0')))
      assert.isTrue(L_SLSD_Asset.gt(toBN('0')))
      ...
      assert.isAtMost(th.getDifference(bob_SLSDDebtRewardSnapshot, L_SLSD), 1000)

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_Asset), 1000)
      assert.isAtMost(th.getDifference(bob_SLSDDebtRewardSnapshot, L_SLSD_Asset), 1000)
    })
    ...
    it("openTrove(): increases the Trove's SLSD debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManagerHelpers.Troves(alice, ZERO_ADDRESS)
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(10000, 18), ZERO_ADDRESS),
        alice,
        alice,
        { from: alice, value: dec(100, 'ether') }
      )
    ...
    it('openTrove(): increases SLSD debt in ActivePool by the debt of the trove', async () => {
      const activePooL_SLSDDebt_Before = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePooL_SLSDDebt_Before_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.equal(activePooL_SLSDDebt_Before, 0)
      assert.equal(activePooL_SLSDDebt_Before_Asset, 0)
      ...
      const activePooL_SLSDDebt_After = await activePool.getSLSDDebt(ZERO_ADDRESS)
      const activePooL_SLSDDebt_After_Asset = await activePool.getSLSDDebt(erc20.address)
      assert.isTrue(activePooL_SLSDDebt_After.eq(aliceDebt))
      assert.isTrue(activePooL_SLSDDebt_After_Asset.eq(aliceDebt_Asset))
    })

    it('openTrove(): increases user SLSDToken balance by correct amount', async () => {
      // check before
      const alice_SLSDTokenBalance_Before = await slsdToken.balanceOf(alice)
      assert.equal(alice_SLSDTokenBalance_Before, 0)
      ...
      // check after
      const alice_SLSDTokenBalance_After = await slsdToken.balanceOf(alice)
      assert.equal(alice_SLSDTokenBalance_After, dec(20000, 18))
    })
    ...
    describe('getNewTCRFromTroveChange() returns the correct TCR', async () => {
      ...
      it('collChange = 0, debtChange = 0', async () => {
        ...
        const troveSLSDAmount = await getOpenTroveSLSDAmount(troveTotalDebt, ZERO_ADDRESS)
        const troveSLSDAmount_Asset = await getOpenTroveSLSDAmount(troveTotalDebt, erc20.address)
        ...
        await borrowerOperations.openTrove(ZERO_ADDRESS, troveColl, th._100pct, troveSLSDAmount, bob, bob, {
          from: bob,
          value: troveColl,
        })
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive ETH', async () => {
        ...
        // Alice sends SLSD to NonPayable so its SLSD balance covers its debt
        await slsdToken.transfer(nonPayable.address, dec(40000, 18), { from: alice })
