...
contract('TroveManager - in Recovery Mode - back to normal mode in 1 tx', async (accounts) => {
  ...
  context('Batch liquidations', () => {
    ...
    it('Stability Pool profit matches', async () => {
      ...
      const spSLSDBefore = await stabilityPool.getTotalSLSDDeposits()

      const spEthBefore_Asset = await stabilityPoolERC20.getAssetBalance()
      const spSLSDBefore_Asset = await stabilityPoolERC20.getTotalSLSDDeposits()
      ...
      const spPSYfter = await stabilityPool.getTotalSLSDDeposits()

      const spEthAfter_Asset = await stabilityPoolERC20.getAssetBalance()
      const spPSYfter_Asset = await stabilityPoolERC20.getTotalSLSDDeposits()
      ...
      const expectedGainInSLSD = expectedCollateralLiquidatedA.mul(price).div(mv._1e18BN).sub(A_totalDebt)
      const realGainInSLSD = spEthAfter
        .sub(spEthBefore)
        .mul(price)
        .div(mv._1e18BN)
        .sub(spSLSDBefore.sub(spPSYfter))

      const expectedGainInSLSD_Asset = expectedCollateralLiquidatedA_Asset
        .mul(price)
        .div(mv._1e18BN)
        .sub(A_totalDebt_Asset)
      const realGainInSLSD_Asset = spEthAfter_Asset
        .sub(spEthBefore_Asset)
        .mul(price)
        .div(mv._1e18BN)
        .sub(spSLSDBefore_Asset.sub(spPSYfter_Asset))
      ...
      assert.equal(
        spSLSDBefore.sub(spPSYfter).toString(),
        A_totalDebt.toString(),
        'Stability Pool SLSD doesn’t match'
      )
      assert.equal(
        realGainInSLSD.toString(),
        expectedGainInSLSD.toString(),
      ...
      assert.equal(
        spSLSDBefore_Asset.sub(spPSYfter_Asset).toString(),
        A_totalDebt_Asset.toString(),
        'Stability Pool SLSD doesn’t match'
      )
      assert.equal(
        realGainInSLSD_Asset.toString(),
        expectedGainInSLSD_Asset.toString(),
        'Stability Pool gains don’t match'
      )
    })
