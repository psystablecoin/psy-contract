...
contract('TroveManager - Redistribution reward calculations', async (accounts) => {
  ...
  it('redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards', async () => {
    ...
    */
    ...
    // check SLSD gas compensation
    assert.equal((await slsdToken.balanceOf(owner)).toString(), toBN(dec(400, 18)).mul(toBN(2)).toString())
  })
  ...
  it('redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves', async () => {
    ...
    */
    ...
    // check SLSD gas compensation
    assert.equal((await slsdToken.balanceOf(owner)).toString(), toBN(dec(1000, 18)).mul(toBN(2)).toString())
  })
  ...
  it('redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt', async () => {
    ...
    // Expect Bob now holds all Ether and SLSDDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 SLSD (10 each for gas compensation)
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()

    const bob_SLSDDebt = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const bob_SLSDDebt_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    assert.isAtMost(
      th.getDifference(bob_SLSDDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    )
  ...
  it('redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.', async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 SLSD
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 SLSDDebt

    coll: 3.1195 ETH
    debt: 233.77 SLSDDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 SLSD
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 SLSDDebt

    coll: 1.8705 ETH
    debt: 146.23 SLSDDebt

    totalColl: 4.99 ETH
    totalDebt 380 SLSD (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const alice_SLSDDebt = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
      .add(await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, alice))
      .toString()
    ...
    const alice_SLSDDebt_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[th.TROVE_DEBT_INDEX]
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_SLSDDebt, expected_B_debt), 10000)

    assert.isAtMost(th.getDifference(bob_Coll_Asset, expected_B_coll_Asset), 1000)
    assert.isAtMost(th.getDifference(bob_SLSDDebt_Asset, expected_B_debt_Asset), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_SLSDDebt, expected_A_debt), 10000)

    assert.isAtMost(th.getDifference(alice_Coll_Asset, expected_A_coll_Asset), 1000)
    assert.isAtMost(th.getDifference(alice_SLSDDebt_Asset, expected_A_debt_Asset), 10000)
  ...
  it('redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards', async () => {
    ...
    */
    ...
    // check SLSD gas compensation
    th.assertIsApproximatelyEqual(
      (await slsdToken.balanceOf(owner)).toString(),
  ...
  it('redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt', async () => {
    ...
    // Expect Bob now holds all Ether and SLSDDebt in the system: 2.5 Ether and 300 SLSD
  ...
  it('redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.', async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 ETH, 55 SLSD
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 SLSDDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 SLSDDebt 

     Alice rewards:
    L1 0.4975, 55 SLSD
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 SLSDDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 SLSD Debt

    totalColl: 3.49 ETH
    totalDebt 380 SLSD (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
      .add(await troveManagerHelpers.getPendingAssetReward(ZERO_ADDRESS, bob))
      .toString()
    ...
    const entireSystemDebt = (await activePool.getSLSDDebt(ZERO_ADDRESS)).add(
      await defaultPool.getSLSDDebt(ZERO_ADDRESS)
    )
    const entireSystemDebt_Asset = (await activePool.getSLSDDebt(erc20.address)).add(
  ...
  it('redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards', async () => {
    ...
    // check SLSD gas compensation
    assert.equal((await slsdToken.balanceOf(owner)).toString(), toBN(dec(600, 18)).mul(toBN(2)).toString())
  })
