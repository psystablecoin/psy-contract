...
const SLSDToken = artifacts.require('SLSDToken')
...
contract('Pool Manager: Sum-Product rounding errors', async (accounts) => {
  ...
  it('Rounding errors: 100 deposits of 100SLSD into SP, then 200 liquidations of 49SLSD', async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalSLSDDeposits() // 0
    console.log(+SP_TotalDeposits)
    const SP_ETH = await stabilityPool.getAssetBalance() // 99,5 ETH
    console.log(+SP_ETH)
    const compoundedDeposit = await stabilityPool.getCompoundedSLSDDeposit(depositors[0])
    ...
    const SP_TotalDepositsERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
    console.log(+SP_TotalDepositsERC20)
    const SP_ETHERC20 = await stabilityPoolERC20.getAssetBalance()
    console.log(+SP_ETHERC20)
    const compoundedDepositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(depositors[0])
