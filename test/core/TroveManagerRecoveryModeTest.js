...
contract('TroveManager - in Recovery Mode', async (accounts) => {
  ...
  it('checkRecoveryMode(): Returns true if TCR falls below CCR', async () => {
    ...
    // price drops to 1ETH:150SLSD, reducing TCR below 150%. setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it('liquidate(), with ICR < 100%: updates system snapshots correctly', async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100SLSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it('liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array', async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraSLSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it('liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool', async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraSLSDAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
  ...
  it('liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes', async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 SLSD of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1ETH:100SLSD, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it('liquidate(), with 100% < ICR < 110%: updates system snapshots correctly', async () => {
    ...
    //  Bob withdraws up to 20000 SLSD of debt, bringing his ICR to 210%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(210, 16)),
      extraSLSDAmount: dec(20000, 18),
      extraParams: { from: bob },
    })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it('liquidate(), with 100% < ICR < 110%: closes the Trove and removes it from the Trove array', async () => {
    ...
    //  Bob withdraws up to 2000 SLSD of debt, bringing his ICR to 210%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(210, 16)), extraSLSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it('liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt', async () => {
    ...
    // Alice deposits 390SLSD to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, { from: alice })
    await stabilityPoolERC20.provideToSP(spDeposit, { from: alice })
    ...
    // check pool SLSD before liquidation
    const stabilityPoolSLSD_Before = (await stabilityPool.getTotalSLSDDeposits()).toString()
    assert.equal(stabilityPoolSLSD_Before, '390000000000000000000')

    const stabilityPoolSLSD_Before_Asset = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
    assert.equal(stabilityPoolSLSD_Before_Asset, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 SLSD.
    
    With 390 SLSD in the StabilityPool, 390 SLSD should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    SLSDLoss: 390SLSD
    AssetGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 SLSD and 4.074525 ether, the remainders - 1610 SLSD and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
    await troveManager.liquidate(erc20.address, bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedSLSDDeposit(alice)
    ...
    const aliceDeposit_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 SLSD and 16.82 ether are distributed.
    ...
    L_SLSDDebt = 1610 / 6 = 268.333 SLSD
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_SLSDDebt = (await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)).toString()
    const L_ETH = (await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)).toString()

    const L_SLSDDebt_Asset = (await troveManagerHelpers.L_SLSDDebts(erc20.address)).toString()
    const L_ETH_Asset = (await troveManagerHelpers.L_ASSETS(erc20.address)).toString()

    assert.isAtMost(
      th.getDifference(L_SLSDDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))),
      100
    )
    assert.isAtMost(
      th.getDifference(
  ...
  it('liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing', async () => {
    ...
    assert.equal(L_SLSDDebt, '0')
    assert.equal(L_ETH, '0')
    assert.equal(L_SLSDDebt_Asset, '0')
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool SLSD is GREATER THAN liquidated debt ---

  it('liquidate(), with 110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: offsets the trove entirely with the pool', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraSLSDAmount: dec(250, 18),
      extraParams: { from: bob },
    })
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: B_totalDebt, extraParams: { from: alice } })
    await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: dec(2000, 18),
      extraParams: { from: dennis },
    ...
    // Alice deposits SLSD in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 SLSD, Alice sole depositor.
    As liquidated debt (250 SLSD) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240SLSD
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedSLSDDeposit(alice)
    const aliceExpectedETHGain = await stabilityPool.getDepositorAssetGain(alice)
    const aliceExpectedDeposit_Asset = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
  ...
  it('liquidate(), with ICR% = 110 < TCR, and StabilityPool SLSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: B_totalDebt_Asset,
      extraParams: { from: alice },
    })
    await openTrove({
      asset: erc20.address,
  ...
  it('liquidate(), with  110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: removes stake and updates totalStakes', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with  110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: updates system snapshots', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with 110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: closes the Trove', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it('liquidate(), with 110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: can liquidate troves out of order', async () => {
    // taking out 1000 SLSD, CR of 200%
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: totalLiquidatedDebt,
      extraParams: { from: whale },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: totalLiquidatedDebt_Asset,
      extraParams: { from: whale },
    })
  ...
  SLSD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it('liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: Trove remains active', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: dec(1500, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(240, 16)), extraSLSDAmount: dec(250, 18), extraParams: { from: bob } })
    await openTrove({
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: dec(2000, 18),
      extraParams: { from: dennis },
    ...
    // Alice deposits 1490 SLSD in the Stability Pool
    ...
    /* Since the pool only contains 100 SLSD, and Bob's pre-liquidation debt was 250 SLSD,
  ...
  it('liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: Trove remains in TroveOwners array', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    // Alice deposits 100 SLSD in the Stability Pool
  ...
  it('liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: nothing happens', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 SLSD) is larger than all SLSD in the Stability Pool, Liquidation won’t happen
  ...
  it('liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: updates system shapshots', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const L_SLSDDebt_After = (await troveManagerHelpers.L_SLSDDebts(ZERO_ADDRESS)).toString()
    const L_ETH_After = (await troveManagerHelpers.L_ASSETS(ZERO_ADDRESS)).toString()

    const L_SLSDDebt_After_Asset = (await troveManagerHelpers.L_SLSDDebts(erc20.address)).toString()
    const L_ETH_After_Asset = (await troveManagerHelpers.L_ASSETS(erc20.address)).toString()

    assert.equal(L_SLSDDebt_After, '0')
    assert.equal(L_ETH_After, '0')
    assert.equal(L_SLSDDebt_After_Asset, '0')
    assert.equal(L_ETH_After_Asset, '0')
  })

  it('liquidate(), with ICR > 110%, and StabilityPool SLSD < liquidated debt: ICR of non liquidated trove does not change', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 SLSD, -> ICR of 250%.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: dec(1500, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(240, 16)), extraSLSDAmount: dec(250, 18), extraParams: { from: bob } })
    await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    await openTrove({ ICR: toBN(dec(250, 16)), extraSLSDAmount: dec(240, 18), extraParams: { from: carol } })
    ...
    //Check SP SLSD has been completely emptied
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), dec(100, 18))
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), dec(100, 18))
    ...
    await slsdToken.transfer(bob, toBN(dec(100, 18)).mul(toBN(2)), { from: alice })
    ...
    // Alice provides another 50 SLSD to pool
    ...
    //Check SP SLSD has been completely emptied
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), dec(150, 18))
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), dec(150, 18))
  ...
  it('liquidate() with ICR > 110%, and StabilityPool SLSD < liquidated debt: total liquidated coll and debt is correct', async () => {
    // Whale provides 50 SLSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraSLSDAmount: dec(50, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: dec(50, 18),
      extraParams: { from: whale },
    })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 SLSD, and provides 10 SLSD to SP
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: alice } })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(200, 16)), extraParams: { from: alice } })
  ...
  it('liquidate(): does nothing if trove has >= 110% ICR and the Stability Pool is empty', async () => {
    ...
    // Confirm SP is empty
    const SLSDinSP = (await stabilityPool.getTotalSLSDDeposits()).toString()
    assert.equal(SLSDinSP, '0')

    const SLSDinSP_Asset = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
    assert.equal(SLSDinSP_Asset, '0')
  ...
  it('liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt', async () => {
    ...
    /*  Since there is 0 SLSD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraSLSDAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(220, 16)),
      extraSLSDAmount: spDeposit_Asset,
      extraParams: { from: bob },
    })

    // Bob sends tokens to Dennis, who has no trove
    await slsdToken.transfer(dennis, spDeposit.add(spDeposit_Asset), { from: bob })

    //Dennis provides 200 SLSD to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraSLSDAmount: dec(1000, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(220, 16)),
      extraSLSDAmount: dec(1000, 18),
      extraParams: { from: whale },
    })

    const { SLSDAmount: A_SLSDAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: dec(300, 18),
      extraParams: { from: alice },
    })
  ...
  it('liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again', async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 SLSD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 SLSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraSLSDAmount: dec(480, 18),
      extraParams: { from: bob },
    })
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 SLSD, ICR = 120% (lowest one)
    ...
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: B_netDebt_2, extraParams: { from: dennis } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: B_netDebt_2_Asset,
      extraParams: { from: dennis },
    })
  ...
  it('liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption', async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 SLSD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraSLSDAmount: dec(90, 18),
      extraParams: { from: bob },
    })
    // Dennis withdraws to 150 SLSD of debt, resulting in ICRs of 266%.
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: B_netDebt, extraParams: { from: dennis } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: B_netDebt_Asset,
      extraParams: { from: dennis },
    })
    ...
    // Bob re-opens the trove, price 200, total debt 250 SLSD, ICR = 240% (lowest one)
    ...
    // Alice deposits SLSD in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraSLSDAmount: B_totalDebt_2, extraParams: { from: alice } })
    await stabilityPool.provideToSP(B_totalDebt_2, { from: alice })

    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(266, 16)),
      extraSLSDAmount: B_totalDebt_2_Asset,
      extraParams: { from: alice },
    })
  ...
  it('liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode', async () => {
    ...
    // Everyone withdraws some SLSD from their Trove, resulting in different ICRs
    ...
    // Alice deposits SLSD to Stability Pool
    ...
    // price drops
    // price drops to 1ETH:90SLSD, reducing TCR below 150%
  ...
  it('liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%', async () => {
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(400, 16)),
      extraSLSDAmount: liquidationAmount_Asset,
      extraParams: { from: alice },
    })
    ...
    // price drops to 1ETH:85SLSD, reducing TCR below 150%
  ...
  it('liquidateTroves(): does nothing if n = 0', async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it('liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: dec(500, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraSLSDAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    })
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraSLSDAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    })
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraSLSDAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    })
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraSLSDAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 SLSD
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 SLSD in Stability Pool
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), '0')
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds SLSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    const spDeposit_Asset = F_totalDebt_Asset.add(G_totalDebt_Asset)
    await openTrove({ ICR: toBN(dec(285, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(285, 16)),
      extraSLSDAmount: spDeposit_Asset,
    ...
    // Confirm SLSD in Stability Pool
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), spDeposit.toString())
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), spDeposit_Asset.toString())
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const entireDebt_A = (await troveManagerHelpers.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, alice)
    )
    ...
    const entireDebt_A_Asset = (await troveManagerHelpers.Troves(alice, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { SLSDAmount: SLSDAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    })
    const { SLSDAmount: SLSDAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    })
    const { SLSDAmount: SLSDAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    })

    const { SLSDAmount: SLSDAmountD_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    })
    const { SLSDAmount: SLSDAmountE_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    })
    const { SLSDAmount: SLSDAmountF_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    })
    ...
    assert.equal((await slsdToken.balanceOf(dennis)).toString(), SLSDAmountD.add(SLSDAmountD_Asset))
    assert.equal((await slsdToken.balanceOf(erin)).toString(), SLSDAmountE.add(SLSDAmountE_Asset))
    assert.equal((await slsdToken.balanceOf(freddy)).toString(), SLSDAmountF.add(SLSDAmountF_Asset))
  ...
  it('liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain', async () => {
    // Whale provides SLSD to the SP
    const { SLSDAmount: W_SLSDAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: dec(4000, 18),
      extraParams: { from: whale },
    })
    const { SLSDAmount: W_SLSDAmount_Asset } = await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
      extraSLSDAmount: dec(4000, 18),
      extraParams: { from: whale },
    ...
    await stabilityPool.provideToSP(W_SLSDAmount, { from: whale })
    await stabilityPoolERC20.provideToSP(W_SLSDAmount_Asset, { from: whale })

    const {
      SLSDAmount: A_SLSDAmount,
    ...
    await stabilityPool.provideToSP(A_SLSDAmount, { from: alice })
    await stabilityPool.provideToSP(B_SLSDAmount, { from: bob })
    await stabilityPoolERC20.provideToSP(A_SLSDAmount_Asset, { from: alice })
    await stabilityPoolERC20.provideToSP(B_SLSDAmount_Asset, { from: bob })

    const totalDeposit = W_SLSDAmount.add(A_SLSDAmount).add(B_SLSDAmount)
    const totalDeposit_Asset = W_SLSDAmount_Asset.add(A_SLSDAmount_Asset).add(B_SLSDAmount_Asset)
    ...
    // Check SLSD in Pool
    assert.equal((await stabilityPool.getTotalSLSDDeposits()).toString(), totalDeposit)
    assert.equal((await stabilityPoolERC20.getTotalSLSDDeposits()).toString(), totalDeposit_Asset)
    ...
    Total SLSD in Pool: 680 SLSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 SLSD
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale SLSD Loss: 500 * (400/680) = 294.12 SLSD
    Alice SLSD Loss:  500 *(40/680) = 29.41 SLSD
    Bob SLSD Loss: 500 * (240/680) = 176.47 SLSD

    Whale remaining deposit: (400 - 294.12) = 105.88 SLSD
    Alice remaining deposit: (40 - 29.41) = 10.59 SLSD
    Bob remaining deposit: (240 - 176.47) = 63.53 SLSD

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 SLSD
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedSLSDDeposit(whale)).toString()
    ...
    const whale_Deposit_After_Asset = (await stabilityPoolERC20.getCompoundedSLSDDeposit(whale)).toString()
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_SLSDAmount.sub(liquidatedDebt.mul(W_SLSDAmount).div(totalDeposit))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_SLSDAmount.sub(liquidatedDebt.mul(A_SLSDAmount).div(totalDeposit))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_SLSDAmount.sub(liquidatedDebt.mul(B_SLSDAmount).div(totalDeposit))
      ),
      100000
    )
    ...
    assert.isAtMost(
      th.getDifference(
        ...
        W_SLSDAmount_Asset.sub(liquidatedDebt_Asset.mul(W_SLSDAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        A_SLSDAmount_Asset.sub(liquidatedDebt_Asset.mul(A_SLSDAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    assert.isAtMost(
      th.getDifference(
        ...
        B_SLSDAmount_Asset.sub(liquidatedDebt_Asset.mul(B_SLSDAmount_Asset).div(totalDeposit_Asset))
      ),
      100000
    )
    ...
    assert.isAtMost(
      th.getDifference(
        whale_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(W_SLSDAmount).div(totalDeposit)
      ),
      2000
    )
    assert.isAtMost(
      th.getDifference(
        alice_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(A_SLSDAmount).div(totalDeposit)
      ),
      2000
    )
    assert.isAtMost(
      th.getDifference(
        bob_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(B_SLSDAmount).div(totalDeposit)
      ),
      2000
    )
    ...
    assert.isAtMost(th.getDifference(total_SLSDinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)

    assert.isAtMost(
      th.getDifference(total_SLSDinSP_Asset, totalDeposit_Asset.sub(liquidatedDebt_Asset)),
      1000
    )
  ...
  it('liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain', async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraSLSDAmount: dec(400, 18), extraParams: { from: whale } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(300, 16)),
    ...
    await openTrove({ ICR: toBN(dec(182, 16)), extraSLSDAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraSLSDAmount: dec(300, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(170, 16)), extraParams: { from: carol } })

    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(182, 16)),
      extraSLSDAmount: dec(170, 18),
      extraParams: { from: alice },
    })
    await openTrove({
      asset: erc20.address,
    ...
    // Check SLSD and ETH in Pool  before
    const SLSDinSP_Before = (await stabilityPool.getTotalSLSDDeposits()).toString()
    const ETHinSP_Before = (await stabilityPool.getAssetBalance()).toString()
    const SLSDinSP_Before_Asset = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
    const ETHinSP_Before_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

    assert.equal(SLSDinSP_Before, dec(800, 18))
    assert.equal(ETHinSP_Before, '0')
    assert.equal(SLSDinSP_Before_Asset, dec(800, 18))
    ...
    // Check SLSD and ETH in Pool after
    const SLSDinSP_After = (await stabilityPool.getTotalSLSDDeposits()).toString()
    const ETHinSP_After = (await stabilityPool.getAssetBalance()).toString()
    const SLSDinSP_After_Asset = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
    const ETHinSP_After_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

    assert.equal(SLSDinSP_Before, SLSDinSP_After)
    assert.equal(ETHinSP_Before, ETHinSP_After)
    assert.equal(SLSDinSP_Before_Asset, SLSDinSP_After_Asset)
  ...
  it('liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active', async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
    ...
    That leaves 50 SLSD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    await troveManager.liquidateTroves(erc20.address, 10)
  ...
  it('liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array', async () => {
    ...
    // Whale provides SLSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
  ...
  it('liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraSLSDAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(200, 16)),
      extraSLSDAmount: D_totalDebt_Asset,
      extraParams: { from: carol },
    })
    await openTrove({ asset: erc20.address, ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 SLSD in the Pool that won’t be enough to absorb Carol,
  ...
  it('liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct', async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 SLSD in the Pool that won’t be enough to absorb any other trove */
    await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    await troveManager.liquidateTroves(erc20.address, 10)

    // Expect system debt reduced by 203 SLSD and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
    const entireSystemCollAfter_Asset = await troveManager.getEntireSystemColl(erc20.address)
    const entireSystemDebtAfter_Asset = await troveManager.getEntireSystemDebt(erc20.address)
  ...
  it('liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values', async () => {
    ...
    // Whale provides SLSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 SLSD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
    const liquidationTx_Asset = await troveManager.liquidateTroves(erc20.address, 10)

    const [liquidatedDebt, liquidatedColl, collGasComp, SLSDGasComp] =
      th.getEmittedLiquidationValues(liquidationTx)
    const [liquidatedDebt_Asset, liquidatedColl_Asset, collGasComp_Asset, SLSDGasComp_Asset] =
    ...
    assert.equal(SLSDGasComp.toString(), dec(400, 18))
    ...
    assert.equal(SLSDGasComp_Asset.toString(), dec(400, 18))
  ...
  it('batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode', async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraSLSDAmount: spDeposit, extraParams: { from: alice } })
  ...
  it('batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode', async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraSLSDAmount: A_totalDebt, extraParams: { from: whale } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(426, 16)),
      extraSLSDAmount: A_totalDebt_Asset,
      extraParams: { from: whale },
    })
    ...
    await slsdToken.transfer(alice, A_totalDebt.add(A_totalDebt_Asset), { from: whale })
    // Alice closes trove
  ...
  it('batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct', async () => {
    ...
    // Expect system debt reduced by 203 SLSD and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
  ...
  it('batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool SLSD > debt to liquidate: can liquidate troves out of order', async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    })
    ...
    await openTrove({ ICR: toBN(dec(280, 16)), extraSLSDAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraSLSDAmount: dec(500, 18), extraParams: { from: freddy } })
    ...
    // Whale provides 1000 SLSD to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManagerHelpers.Troves(dennis, ZERO_ADDRESS))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, dennis))
    const bobDebt_After = (await troveManagerHelpers.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, bob)
    )
    const carolDebt_After = (await troveManagerHelpers.Troves(carol, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX].add(
      await troveManagerHelpers.getPendingSLSDDebtReward(ZERO_ADDRESS, carol)
    )
    ...
    const dennisDebt_After_Asset = (await troveManagerHelpers.Troves(dennis, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, dennis))
    const bobDebt_After_Asset = (await troveManagerHelpers.Troves(bob, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, bob))
    const carolDebt_After_Asset = (await troveManagerHelpers.Troves(carol, erc20.address))[
      th.TROVE_DEBT_INDEX
    ].add(await troveManagerHelpers.getPendingSLSDDebtReward(erc20.address, carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    const spDeposit = A_totalDebt.add(C_totalDebt)
      ...
      .add(H_totalDebt)
      .add(I_totalDebt)
    await openTrove({ ICR: toBN(dec(245, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 SLSD
    ...
    // Expected Stability Pool to fully absorb B (92 SLSD + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalSLSDDeposits()
    const dEbtBefore = (await troveManagerHelpers.Troves(E, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

    const stabilityBefore_Asset = await stabilityPoolERC20.getTotalSLSDDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalSLSDDeposits()

    const dEbtAfter_Asset = (await troveManagerHelpers.Troves(E, erc20.address))[th.TROVE_DEBT_INDEX]
    const stabilityAfter_Asset = await stabilityPoolERC20.getTotalSLSDDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 SLSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraSLSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, { from: whale })
