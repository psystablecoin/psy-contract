...
contract('StabilityPool', async (accounts) => {
  ...
  describe('Stability Pool Mechanisms', async () => {
    ...
    it('withdrawFromSP(): partial retrieval - retrieves correct SLSD amount and the entire ETH Gain, and updates deposit', async () => {
      // --- SETUP ---
      // Whale deposits 185000 SLSD in StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(2, 23)),
        ICR: toBN(dec(5, 18)),
      ...
      // Alice makes deposit #1: 15000 SLSD
      await openTrove({
      ...
      // 2 users with Trove with 170 SLSD drawn are closed
      ...
      // Alice SLSDLoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedSLSDLoss_A = liquidatedDebt_1
        .mul(toBN(dec(15000, 18)))
        .div(toBN(dec(200000, 18)))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedSLSDLoss_AERC20 = liquidatedDebt_1ERC20
        .mul(toBN(dec(15000, 18)))
        .div(toBN(dec(200000, 18)))
        .add(liquidatedDebt_2ERC20.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedCompoundedSLSDDeposit_A = toBN(dec(15000, 18)).sub(expectedSLSDLoss_A)
      const compoundedSLSDDeposit_A = await stabilityPool.getCompoundedSLSDDeposit(alice)

      const expectedCompoundedSLSDDeposit_AERC20 = toBN(dec(15000, 18)).sub(expectedSLSDLoss_AERC20)
      const compoundedSLSDDeposit_AERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)

      assert.isAtMost(th.getDifference(expectedCompoundedSLSDDeposit_A, compoundedSLSDDeposit_A), 100000)
      assert.isAtMost(
        th.getDifference(expectedCompoundedSLSDDeposit_AERC20, compoundedSLSDDeposit_AERC20),
        100000
      )

      // Alice retrieves part of her entitled SLSD: 9000 SLSD
      await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice })
      await stabilityPoolERC20.withdrawFromSP(dec(9000, 18), { from: alice })

      const expectedNewDeposit_A = compoundedSLSDDeposit_A.sub(toBN(dec(9000, 18)))
      const expectedNewDeposit_AERC20 = compoundedSLSDDeposit_AERC20.sub(toBN(dec(9000, 18)))
    ...
    it('withdrawFromSP(): partial retrieval - leaves the correct amount of SLSD in the Stability Pool', async () => {
      // --- SETUP ---
      // Whale deposits 185000 SLSD in StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(2, 23)),
      ...
      const SP_SLSD_Before = await stabilityPool.getTotalSLSDDeposits()
      assert.equal(SP_SLSD_Before, dec(200000, 18))

      const SP_SLSD_BeforeERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.equal(SP_SLSD_BeforeERC20, dec(200000, 18))
      ...
      Expect SLSD in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPSLSD = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1))
        .sub(toBN(liquidatedDebt_2))
        .sub(toBN(dec(9000, 18)))

      const expectedSPSLSDERC20 = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1ERC20))
        .sub(toBN(liquidatedDebt_2ERC20))
        .sub(toBN(dec(9000, 18)))

      const SP_SLSD_After = (await stabilityPool.getTotalSLSDDeposits()).toString()
      const SP_SLSD_AfterERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()

      th.assertIsApproximatelyEqual(SP_SLSD_After, expectedSPSLSD)
      th.assertIsApproximatelyEqual(SP_SLSD_AfterERC20, expectedSPSLSDERC20)
    })

    it('withdrawFromSP(): full retrieval - leaves the correct amount of SLSD in the Stability Pool', async () => {
      // --- SETUP ---
      // Whale deposits 185000 SLSD in StabilityPool
      await openTrove({
      ...
      const SLSDinSPBefore = await stabilityPool.getTotalSLSDDeposits()
      const SLSDinSPBeforeERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()

      // Alice retrieves all of her entitled SLSD:
      ...
      const expectedSLSDinSPAfter = SLSDinSPBefore.sub(compoundedSLSDDeposit_A)
      const expectedSLSDinSPAfterERC20 = SLSDinSPBefore.sub(compoundedSLSDDeposit_AERC20)

      const SLSDinSPAfter = await stabilityPool.getTotalSLSDDeposits()
      const SLSDinSPAfterERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.isAtMost(th.getDifference(expectedSLSDinSPAfter, SLSDinSPAfter), 100000)
      assert.isAtMost(th.getDifference(expectedSLSDinSPAfterERC20, SLSDinSPAfterERC20), 100000)
    })
    ...
    it('withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero ETH', async () => {
      // --- SETUP ---
      // Whale deposits 1850 SLSD in StabilityPool
      await openTrove({
    ...
    it("withdrawFromSP(): it correctly updates the user's SLSD and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 SLSD in StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(1000000, 18)),
    ...
    it('withdrawFromSP(): All depositors are able to withdraw from the SP to their account', async () => {
      ...
      const totalDeposits = (await stabilityPool.getTotalSLSDDeposits()).toString()
      const totalDepositsERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()

      assert.isAtMost(th.getDifference(totalDeposits, '0'), 100000)
      assert.isAtMost(th.getDifference(totalDepositsERC20, '0'), 100000)
    })

    it("withdrawFromSP(): increases depositor's SLSD token balance by the expected amount", async () => {
      // Whale opens trove
      await openTrove({
      ...
      const aliceBalBefore = await slsdToken.balanceOf(alice)
      const bobBalBefore = await slsdToken.balanceOf(bob)

      /* From an offset of 10000 SLSD, each depositor receives
      SLSDLoss = 1666.6666666666666666 SLSD

      and thus with a deposit of 10000 SLSD, each should withdraw 8333.3333333333333333 SLSD (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18))

      // Bob issues a further 5000 SLSD from his trove
      await borrowerOperations.withdrawSLSD(ZERO_ADDRESS, th._100pct, dec(5000, 18), bob, bob, { from: bob })
      await borrowerOperations.withdrawSLSD(erc20.address, th._100pct, dec(5000, 18), bob, bob, { from: bob })

      // Expect Alice's SLSD balance increase be very close to 8333.3333333333333333 SLSD
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      await stabilityPoolERC20.withdrawFromSP(dec(10000, 18), { from: alice })
      const aliceBalance = await slsdToken.balanceOf(alice)
      ...
      // expect Bob's SLSD balance increase to be very close to  13333.33333333333333333 SLSD
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      await stabilityPoolERC20.withdrawFromSP(dec(10000, 18), { from: bob })
      const bobBalance = await slsdToken.balanceOf(bob)
      assert.isAtMost(
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or ETH gains", async () => {
      ...
      const alice_SLSDDeposit_Before = (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString()
      const bob_SLSDDeposit_Before = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()

      const alice_SLSDDeposit_BeforeERC20 = (
        await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      ).toString()
      const bob_SLSDDeposit_BeforeERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
      ...
      const alice_SLSDDeposit_After = (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString()
      const bob_SLSDDeposit_After = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()
      ...
      const alice_SLSDDeposit_AfterERC20 = (
        await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      ).toString()
      const bob_SLSDDeposit_AfterERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
      ...
      assert.equal(alice_SLSDDeposit_Before, alice_SLSDDeposit_After)
      assert.equal(bob_SLSDDeposit_Before, bob_SLSDDeposit_After)
      assert.equal(alice_ETHGain_Before, alice_ETHGain_After)
      assert.equal(bob_ETHGain_Before, bob_ETHGain_After)

      assert.equal(alice_SLSDDeposit_BeforeERC20, alice_SLSDDeposit_AfterERC20)
      assert.equal(bob_SLSDDeposit_BeforeERC20, bob_SLSDDeposit_AfterERC20)
    ...
    it("withdrawFromSP(): doesn't impact system debt, collateral or TCR ", async () => {
      ...
      const activeDebt_Before = (await activePool.getSLSDDebt(ZERO_ADDRESS)).toString()
      const defaultedDebt_Before = (await defaultPool.getSLSDDebt(ZERO_ADDRESS)).toString()
      ...
      const activeDebt_BeforeERC20 = (await activePool.getSLSDDebt(erc20.address)).toString()
      const defaultedDebt_BeforeERC20 = (await defaultPool.getSLSDDebt(erc20.address)).toString()
      ...
      const activeDebt_After = (await activePool.getSLSDDebt(ZERO_ADDRESS)).toString()
      const defaultedDebt_After = (await defaultPool.getSLSDDebt(ZERO_ADDRESS)).toString()
      ...
      const activeDebt_AfterERC20 = (await activePool.getSLSDDebt(erc20.address)).toString()
      const defaultedDebt_AfterERC20 = (await defaultPool.getSLSDDebt(erc20.address)).toString()
    ...
    it('withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove', async () => {
      ...
      const A_PSYBalBefore = await psyToken.balanceOf(A)

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorAssetGain(A)
      const A_pendingPSYGain = await stabilityPool.getDepositorPSYGain(A)
      assert.isTrue(A_pendingETHGain.gt(toBN('0')))
      assert.isTrue(A_pendingPSYGain.gt(toBN('0')))

      const A_pendingETHGainERC20 = await stabilityPoolERC20.getDepositorAssetGain(A)
      const A_pendingPSYGainERC20 = await stabilityPoolERC20.getDepositorPSYGain(A)
      assert.isTrue(A_pendingETHGainERC20.gt(toBN('0')))
      assert.isTrue(A_pendingPSYGainERC20.gt(toBN('0')))
      ...
      const A_PSYBalAfter = await psyToken.balanceOf(A)
      const A_PSYBalDiff = A_PSYBalAfter.sub(A_PSYBalBefore)

      // Check A's ETH and PSY balances have increased correctly
      ...
      assert.isAtMost(th.getDifference(A_PSYBalDiff, A_pendingPSYGain.add(A_pendingPSYGainERC20)), 1000)
    })

    it("withdrawFromSP(): withdrawing 0 SLSD doesn't alter the caller's deposit or the total SLSD in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // A, B, C provides 100, 50, 30 SLSD to SP
      await stabilityPool.provideToSP(dec(100, 18), { from: alice })
      ...
      const bob_Deposit_BeforeERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
      const SLSDinSP_BeforeERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()

      assert.equal(SLSDinSP_Before, dec(180, 18))
      assert.equal(SLSDinSP_BeforeERC20, dec(180, 18))

      // Bob withdraws 0 SLSD from the Stability Pool
      await stabilityPool.withdrawFromSP(0, { from: bob })
      await stabilityPoolERC20.withdrawFromSP(0, { from: bob })

      // check Bob's deposit and total SLSD in Stability Pool has not changed
      ...
      const bob_Deposit_AfterERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
      const SLSDinSP_AfterERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()

      assert.equal(bob_Deposit_Before, bob_Deposit_After)
      assert.equal(SLSDinSP_Before, SLSDinSP_After)

      assert.equal(bob_Deposit_BeforeERC20, bob_Deposit_AfterERC20)
      assert.equal(SLSDinSP_BeforeERC20, SLSDinSP_AfterERC20)
    })
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_SLSD_Balance_Before = await slsdToken.balanceOf(alice)
      const bob_SLSD_Balance_Before = await slsdToken.balanceOf(bob)

      const alice_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(bob)

      const alice_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      const bob_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)

      const SLSDinSP_Before = await stabilityPool.getTotalSLSDDeposits()
      const SLSDinSP_BeforeERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      ...
      // Check Bob's SLSD balance has risen by only the value of his compounded deposit
      const bob_expectedSLSDBalance = bob_SLSD_Balance_Before
        .add(bob_Deposit_Before)
        .add(bob_Deposit_BeforeERC20)
        .toString()
      const bob_SLSD_Balance_After = (await slsdToken.balanceOf(bob)).toString()
      assert.equal(bob_SLSD_Balance_After, bob_expectedSLSDBalance)

      // Alice attempts to withdraws 2309842309.000000000000000000 SLSD from the Stability Pool
      ...
      // Check Alice's SLSD balance has risen by only the value of her compounded deposit
      const alice_expectedSLSDBalance = alice_SLSD_Balance_Before
        .add(alice_Deposit_Before)
        .add(alice_Deposit_BeforeERC20)
        .toString()
      const alice_SLSD_Balance_After = (await slsdToken.balanceOf(alice)).toString()
      assert.equal(alice_SLSD_Balance_After, alice_expectedSLSDBalance)

      // Check SLSD in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedSLSDinSP = SLSDinSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before).toString()
      const SLSDinSP_After = (await stabilityPool.getTotalSLSDDeposits()).toString()
      assert.equal(SLSDinSP_After, expectedSLSDinSP)
    })

    it("withdrawFromSP(): Request to withdraw 2^256-1 SLSD only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // Bob attempts to withdraws maxBytes32 SLSD from the Stability Pool
      ...
      // Check SLSD in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedSLSDinSP = SLSDinSP_Before.sub(bob_Deposit_Before).toString()
      const SLSDinSP_After = (await stabilityPool.getTotalSLSDDeposits()).toString()
      assert.equal(SLSDinSP_After, expectedSLSDinSP)

      const expectedSLSDinSPERC20 = SLSDinSP_BeforeERC20.sub(bob_Deposit_BeforeERC20).toString()
      const SLSDinSP_AfterERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
      assert.equal(SLSDinSP_AfterERC20, expectedSLSDinSPERC20)
    })
    ...
    it('withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode', async () => {
      ...
      // A, B, C provides 10000, 5000, 3000 SLSD to SP
      await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
      ...
      const alice_SLSD_Balance_Before = await slsdToken.balanceOf(alice)
      const bob_SLSD_Balance_Before = await slsdToken.balanceOf(bob)
      const carol_SLSD_Balance_Before = await slsdToken.balanceOf(carol)
      ...
      const alice_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(bob)
      const carol_Deposit_Before = await stabilityPool.getCompoundedSLSDDeposit(carol)

      const alice_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      const bob_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
      const carol_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)
      ...
      // Check SLSD balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedSLSDBalance = alice_SLSD_Balance_Before
        .add(alice_Deposit_Before)
        .add(alice_Deposit_BeforeERC20)
        .toString()
      const bob_expectedSLSDBalance = bob_SLSD_Balance_Before
        ...
        .add(bob_Deposit_BeforeERC20)
        .toString()
      const carol_expectedSLSDBalance = carol_SLSD_Balance_Before
      ...
      const alice_SLSD_Balance_After = (await slsdToken.balanceOf(alice)).toString()
      const bob_SLSD_Balance_After = (await slsdToken.balanceOf(bob)).toString()
      const carol_SLSD_Balance_After = (await slsdToken.balanceOf(carol)).toString()

      assert.equal(alice_SLSD_Balance_After, alice_expectedSLSDBalance)
      assert.equal(bob_SLSD_Balance_After, bob_expectedSLSDBalance)
      assert.equal(carol_SLSD_Balance_After, carol_expectedSLSDBalance)
      ...
      // Check SLSD in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedSLSDinSP = SLSDinSP_Before.sub(alice_Deposit_Before)
      ...
      const expectedSLSDinSPERC20 = SLSDinSP_BeforeERC20.sub(alice_Deposit_BeforeERC20)
    ...
    it('getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ', async () => {
      ...
      // A, B, provide 10000, 5000 SLSD to SP
      await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
      await stabilityPool.provideToSP(dec(5000, 18), { from: bob })
      ...
      const SLSDinSPERC20 = (await stabilityPoolERC20.getTotalSLSDDeposits()).toString()
      assert.equal(SLSDinSPERC20, '0')

      // Check Stability deposits have been fully cancelled with debt, and are now all zero
      const alice_Deposit = (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString()
      const bob_Deposit = (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString()

      const alice_DepositERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString()
      const bob_DepositERC20 = (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString()
      ...
      // Whale deposits 10000 SLSD to Stability Pool
    ...
    // --- PSY functionality ---
    it('withdrawFromSP(): triggers PSY reward event - increases the sum G', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(1, 24)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it('withdrawFromSP(), partial withdrawal: depositor receives PSY rewards', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Get A, B, C PSY balance before
      const A_PSYBalance_Before = await psyToken.balanceOf(A)
      const B_PSYBalance_Before = await psyToken.balanceOf(B)
      const C_PSYBalance_Before = await psyToken.balanceOf(C)
      ...
      // Get PSY balance after
      const A_PSYBalance_After = await psyToken.balanceOf(A)
      const B_PSYBalance_After = await psyToken.balanceOf(B)
      const C_PSYBalance_After = await psyToken.balanceOf(C)

      // Check PSY Balance of A, B, C has increased
      assert.isTrue(A_PSYBalance_After.gt(A_PSYBalance_Before))
      assert.isTrue(B_PSYBalance_After.gt(B_PSYBalance_Before))
      assert.isTrue(C_PSYBalance_After.gt(C_PSYBalance_Before))
    })
    ...
    it("withdrawFromSP(), partial withdrawal: System's snapshots update", async () => {
      ...
      await openTrove({
        extraSLSDAmount: toBN(dec(60000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      })
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      // Fast-forward time and make a second deposit, to trigger PSY reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await stabilityPool.provideToSP(dec(10000, 18), { from: E })
      await stabilityPoolERC20.provideToSP(dec(10000, 18), { from: E })
      ...
      await slsdToken.transfer(A, dec(20000, 18), { from: whale })
      await slsdToken.transfer(B, dec(40000, 18), { from: whale })
    ...
    it('withdrawFromSP(), reverts when initial deposit value is 0', async () => {
      ...
      await openTrove({
        extraSLSDAmount: toBN(dec(10100, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      })
      await stabilityPool.provideToSP(dec(10000, 18), { from: A })
      ...
      //  SETUP: Execute a series of operations to trigger PSY and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies SLSDLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 SLSD in StabilityPool
      await openTrove({
        extraSLSDAmount: toBN(dec(1000000, 18)),
      ...
      const compoundedDeposit_A = await stabilityPool.getCompoundedSLSDDeposit(alice)

      const ETHGain_AERC20 = await stabilityPoolERC20.getDepositorAssetGain(alice)
      const compoundedDeposit_AERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      ...
      const expectedSLSDLoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedSLSDLoss_A)

      const expectedETHGain_AERC20 = liquidatedCollERC20.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedSLSDLoss_AERC20 = liquidatedDebtERC20.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedCompoundedDeposit_AERC20 = toBN(dec(15000, 18)).sub(expectedSLSDLoss_AERC20)
      ...
      // check Alice's SLSDLoss has been applied to her deposit expectedCompoundedDeposit_A
    ...
    it('withdrawETHGainToTrove(): reverts if user has no trove', async () => {
      ...
      // A transfers SLSD to D
      await slsdToken.transfer(dennis, dec(20000, 18), { from: alice })
    ...
    it('withdrawETHGainToTrove(): triggers PSY reward event - increases the sum G', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it('withdrawETHGainToTrove(), eligible deposit: depositor receives PSY rewards', async () => {
      await openTrove({
        extraSLSDAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      ...
      // Get PSY balance after
      const A_PSYBalance_AfterERC20 = await psyToken.balanceOf(A)
      const B_PSYBalance_AfterERC20 = await psyToken.balanceOf(B)
      const C_PSYBalance_AfterERC20 = await psyToken.balanceOf(C)

      // Check PSY Balance of A, B, C has increased
      assert.isTrue(A_PSYBalance_AfterERC20.gt(A_PSYBalance_After))
      assert.isTrue(B_PSYBalance_AfterERC20.gt(B_PSYBalance_After))
      assert.isTrue(C_PSYBalance_AfterERC20.gt(C_PSYBalance_After))
    })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: System's snapshots update", async () => {
      ...
      // between topups, because some block.timestamp time passes (and PSY is issued) between ops
      const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch)
      await stabilityPool.withdrawAssetGainToTrove(A, A, { from: A })
    ...
    it('withdrawETHGainToTrove(): reverts when depositor has no ETH gain', async () => {
      ...
      await openTrove({
        extraSLSDAmount: toBN(dec(4000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D },
      })
      ...
      // fastforward time, and E makes a deposit, creating PSY rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await openTrove({
        extraSLSDAmount: toBN(dec(3000, 18)),
        ICR: toBN(dec(2, 18)),
