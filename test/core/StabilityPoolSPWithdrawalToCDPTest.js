  ...
  describe('Stability Pool Withdrawal', async () => {
    ...
    it('withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(100000, 18), ZERO_ADDRESS),
        whale,
        whale,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100000, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(100000, 18), erc20.address),
        whale,
        whale,
        { from: whale }
      )
      ...
      // Whale transfers 10k SLSD to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol]
      for (account of depositors) {
        await SLSDToken.transfer(account, dec(20000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(10000, 18), { from: account })
        await stabilityPoolERC20.provideToSP(dec(10000, 18), { from: account })
      }

      // Defaulter opens trove with 200% ICR and 10k SLSD net debt
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      // Check depositors' compounded deposit is 6666.66 SLSD and ETH Gain is 33.16 ETH
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(alice)).toString(),
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(),
          '6666666666666666666666'
        ),
        10000
      )
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString(),
          '6666666666666666666666'
        ),
        10000
      )
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString(),
          '6666666666666666666666'
        ),
        10000
      )
      assert.isAtMost(
        th.getDifference(
          (await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString(),
          '6666666666666666666666'
        ),
        10000
      )
      assert.isAtMost(
        th.getDifference(
          (await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString(),
          '6666666666666666666666'
        ),
        10000
      )
    ...
    it('withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations', async () => {
      ...
      // Check depositors' compounded deposit is 3333.33 SLSD and ETH Gain is 66.33 ETH
      const txA = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txB = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
    ...
    it('withdrawAssetGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations', async () => {
      ...
      // Check depositors' compounded deposit is 0 SLSD and ETH Gain is 99.5 ETH
      const txA = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: alice })
      const txB = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: bob })
      const txC = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, { from: carol })
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(alice)).toString(), '0'),
        10000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(), '0'),
        10000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(carol)).toString(), '0'),
        10000
      )

      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString(), '0'),
        10000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString(), '0'),
        10000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString(), '0'),
        10000
      )
    ...
    it('withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing SLSD', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(5000, 18)),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(7000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveSLSDAmount(dec(5000, 18), erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveSLSDAmount(dec(7000, 18), erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
    ...
    it('withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing SLSD', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(6000, 18)),
        defaulter_2,
        defaulter_2,
    ...
    it('withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations', async () => {
      ...
      // Whale transfers 10k, 20k, 30k SLSD to A, B and C respectively who then deposit it to the SP
      await SLSDToken.transfer(alice, dec(10000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
      await SLSDToken.transfer(bob, dec(20000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(20000, 18), { from: bob })
      await SLSDToken.transfer(carol, dec(30000, 18), { from: whale })
    ...
    it('withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1000000, 18), ZERO_ADDRESS),
        whale,
        whale,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(1000000, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1000000, 18), erc20.address),
        whale,
        whale,
        { from: whale }
      )
      ...
      Bob:  456000 SLSD
      Carol: 13100 SLSD */
      // Whale transfers SLSD to  A, B and C respectively who then deposit it to the SP
      await SLSDToken.transfer(alice, dec(2000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(2000, 18), { from: alice })
      await SLSDToken.transfer(bob, dec(456000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(456000, 18), { from: bob })
      await SLSDToken.transfer(carol, dec(13100, 18), { from: whale })
      ...
      Defaulter 1: 207000 SLSD & 2160 ETH
      Defaulter 2: 5000 SLSD & 50 ETH
      Defaulter 3: 46700 SLSD & 500 ETH
      */
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('207000000000000000000000'),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(5, 21)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('46700000000000000000000'),
        defaulter_3,
        defaulter_3,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(2160, 18),
        th._100pct,
        await getOpenTroveSLSDAmount('207000000000000000000000', erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      await borrowerOperations.openTrove(
        ...
        dec(50, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(5, 21), erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      await borrowerOperations.openTrove(
        ...
        dec(500, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('46700000000000000000000', erc20.address),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3 }
      )
    ...
    it('withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      await SLSDToken.transfer(dennis, dec(20000, 18), { from: whale })
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(),
          '5000000000000000000000'
        ),
        100000
      )
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString(),
          '5000000000000000000000'
        ),
        100000
      )
    ...
    it('withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(), '0'),
        100000
      )
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString(), '0'),
        100000
      )
    ...
    it('withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 1m ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      */
      // Whale transfers SLSD to  A, B and C respectively who then deposit it to the SP
      await SLSDToken.transfer(alice, dec(60000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(60000, 18), { from: alice })
      await SLSDToken.transfer(bob, dec(20000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(20000, 18), { from: bob })
      await SLSDToken.transfer(carol, dec(15000, 18), { from: whale })
      ...
      /* Defaulters open troves:
      Defaulter 1:  10000 SLSD, 100 ETH
      Defaulter 2:  25000 SLSD, 250 ETH
      Defaulter 3:  5000 SLSD, 50 ETH
      Defaulter 4:  40000 SLSD, 400 ETH
      */
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(25000, 18), ZERO_ADDRESS),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(5000, 18), ZERO_ADDRESS),
        defaulter_3,
        defaulter_3,
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(40000, 18), ZERO_ADDRESS),
        defaulter_4,
        defaulter_4,
      ...
      await borrowerOperations.openTrove(
        ...
        th._100pct,
        await getOpenTroveSLSDAmount(dec(25000, 18), erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      ...
      await borrowerOperations.openTrove(
        ...
        dec(400, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(40000, 18), erc20.address),
        defaulter_4,
        defaulter_4,
        { from: defaulter_4 }
      )
      ...
      // Dennis provides 25000 SLSD
      await SLSDToken.transfer(dennis, dec(25000, 18), { from: whale })
      await SLSDToken.transfer(dennis, dec(25000, 18), { from: whale })
    ...
    it('withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 SLSD.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
    ...
    it('withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      Dennis: 40000 SLSD
      */
      // Whale transfers SLSD to  A, B,C and D respectively who then deposit it to the SP
      await SLSDToken.transfer(alice, dec(20000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(20000, 18), { from: alice })
      await SLSDToken.transfer(bob, dec(25000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(25000, 18), { from: bob })
      await SLSDToken.transfer(carol, dec(12500, 18), { from: whale })
      await stabilityPool.provideToSP(dec(12500, 18), { from: carol })
      await SLSDToken.transfer(dennis, dec(40000, 18), { from: whale })
      ...
      /* Defaulters open troves:
      Defaulter 1: 10000 SLSD
      Defaulter 2: 20000 SLSD
      Defaulter 3: 30000 SLSD
      Defaulter 4: 5000 SLSD
      */
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      assert.isAtMost(
        th.getDifference(
          (await SLSDToken.balanceOf(dennis)).toString(),
    ...
    it('withdrawAssetGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 SLSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct SLSD deposit and ETH Gain', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      // Whale transfers 10k SLSD to A, B and D who then deposit it to the SP
      ...
      // Carol makes deposit
      await SLSDToken.transfer(carol, dec(10000, 18), { from: whale })
    ...
    // A, B withdraw 0SLSD & 100e
    // C, D withdraw 5000SLSD  & 500e
    it('withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
      ...
      // Whale transfers 10k SLSD to A, B who then deposit it to the SP
      ...
      // Defaulter 1 liquidated. 20000 SLSD fully offset with pool.
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_1, { from: owner })

      // Carol, Dennis each deposit 10000 SLSD
      ...
      // Defaulter 2 liquidated. 10000 SLSD offset
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
      ...
      // Expect Alice And Bob's compounded deposit to be 0 SLSD
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(alice)).toString(), '0'),
        10000
      )
      ...
      // Expect Carol And Dennis' compounded deposit to be 50 SLSD
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(carol)).toString(),
          '5000000000000000000000'
    ...
    it('withdrawAssetGainToTrove(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18', async () => {
      ...
      // Defaulter 1 liquidated. 10--0 SLSD fully offset, Pool remains non-zero
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_1, { from: owner })
      ...
      // Defaulter 2 liquidated. 1--00 SLSD, empties pool
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
      ...
      // Defaulter 3 liquidated. 10000 SLSD fully offset, Pool remains non-zero
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_3, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_3, { from: owner })
      ...
      // Defaulter 4 liquidated. 10000 SLSD, empties pool
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_4, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_4, { from: owner })
    ...
    // A, B withdraw 0 SLSD & 100e
    // C, D withdraw 5000 SLSD  & 50e
    it('withdrawAssetGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool', async () => {
      ...
      // Carol, Dennis, Erin each deposit 10000, 20000, 30000 SLSD respectively
      await SLSDToken.transfer(carol, dec(10000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(10000, 18), { from: carol })
      await SLSDToken.transfer(dennis, dec(20000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(20000, 18), { from: dennis })
      await SLSDToken.transfer(erin, dec(30000, 18), { from: whale })
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(erin)).toString(),
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPoolERC20.getCompoundedSLSDDeposit(erin)).toString(),
          '25000000000000000000000'
        ),
        100000
      )
    ...
    // A deposits 10000
    // L1, L2, L3 liquidated with 10000 SLSD each
    // A withdraws all
    ...
    it('withdrawAssetGainToTrove(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation', async () => {
      ...
      // Defaulter 1,2,3 withdraw 10000 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
    ...
    // A,B deposit 10000 SLSD
    // L1 cancels 20000 SLSD, 2E
    // B,C deposits 10000 SLSD
    // L2 cancels 20000 SLSD, 2E
    // E,F deposit 10000 SLSD
    // L3 cancels 20000, 200E
    // G,H deposits 10000
    // L4 cancels 20000, 200E

    // Expect all depositors withdraw 0 SLSD and 100 ETH
    ...
    it('withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool', async () => {
      ...
      // Alice, Bob each deposit 10k SLSD
      ...
      // Erin, Flyn each deposit 10000 SLSD
      ...
      // Defaulter 3 liquidated. 10000 SLSD offset
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_3, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_3, { from: owner })

      // Graham, Harriet each deposit 10000 SLSD
      ...
      // Defaulter 4 liquidated. 10k SLSD offset
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_4, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_4, { from: owner })
      ...
      // Expect all deposits to be 0 SLSD
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(alice)).toString(), '0'),
        100000
      )
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(erin)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(flyn)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(graham)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(harriet)).toString(), '0'),
        100000
      )
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(erin)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(flyn)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(graham)).toString(), '0'),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(harriet)).toString(), '0'),
        100000
      )
    ...
    // A withdraws all
    // B deposits 10000
    // L2 of 9900 SLSD, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
    ...
    it('withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10000 SLSD:  9999.99991 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('9999999910000000000000', ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        dec(100, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('9999999910000000000000', erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      ...
      // Defaulter 2 withdraws 9900 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(9900, 18)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(60, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(9900, 18), erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      ...
      // Defaulter 2 liquidated.  9900 SLSD liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
      await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
      await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
      ...
      // Expect Bob to retain 1% of initial deposit (100 SLSD) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(),
    ...
    // A deposits 10000
    // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 SLSD
    // A withdraws all
    ...
    it('withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10k SLSD.
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      // Defaulter 2 withdraws 59400 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('59400000000000000000000', ZERO_ADDRESS),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(330, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('59400000000000000000000', erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      ...
      Bob:  1000 SLSD, 55 Ether
      Carol:  2000 SLSD, 110 Ether
      Dennis:  3000 SLSD, 165 Ether
     
      Total: 6000 SLSD, 300 Ether
      */
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(), dec(100, 18)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(carol)).toString(), dec(200, 18)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(), dec(300, 18)),
        100000
      )

      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString(), dec(100, 18)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString(), dec(200, 18)),
        100000
      )
      assert.isAtMost(
        th.getDifference(
    ...
    // A make deposit 10000 SLSD
    // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 SLSD
    // A withdraws
    // B makes deposit 10000 SLSD
    // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 SLSD total deposits)
    // B withdraws
    ...
    it('withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      // Defaulter 1 and default 2 each withdraw 9999.999999999 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(99999, 17), ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(99999, 17), erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 SLSD and the full ETH gain of 100 ether
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(), dec(1, 17)),
        100000
      )
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 17)), 100000000000)

      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)).toString(), dec(1, 17)),
        100000
      )
    ...
    // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 SLSD total deposits)
    // B withdraws
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire ETH gain from L2
    it('withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      ...
      // Defaulter 1 and default 2 withdraw up to debt of 9999.9 SLSD and 59999.4 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('9999900000000000000000', ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('59999400000000000000000', ZERO_ADDRESS),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(100, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('9999900000000000000000', erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      await borrowerOperations.openTrove(
        ...
        dec(600, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('59999400000000000000000', erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      ...
      // B, C, D deposit 10000, 20000, 30000 SLSD
      await SLSDToken.transfer(bob, dec(10000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(10000, 18), { from: bob })
      await SLSDToken.transfer(carol, dec(20000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(20000, 18), { from: carol })
      await SLSDToken.transfer(dennis, dec(30000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(30000, 18), { from: dennis })
      ...
      // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} SLSD
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(bob)).toString(), dec(1, 17)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(carol)).toString(), dec(2, 17)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(), dec(3, 17)),
        100000
      )
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(carol)).toString(), dec(2, 17)),
        100000
      )
      assert.isAtMost(
        th.getDifference((await stabilityPoolERC20.getCompoundedSLSDDeposit(dennis)).toString(), dec(3, 17)),
        100000
      )
    ...
    // A make deposit 10000 SLSD
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 SLSD
    ...
    it("withdrawAssetGainToTrove(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      // Defaulters 1 withdraws 9999.9999999 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount('9999999999900000000000', ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        dec(100, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount('9999999999900000000000', erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      ...
      assert.equal((await stabilityPool.getCompoundedSLSDDeposit(alice)).toString(), 0)
      assert.equal((await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)).toString(), 0)
    })

    // --- Serial scale changes ---

    /* A make deposit 10000 SLSD
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 SLSD, 1 ETH
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 SLSD, 1 ETH
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 SLSD, 1 ETH
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 SLSD, 1 ETH
    expect A, B, C, D each withdraw ~100 Ether
    */
    it('withdrawAssetGainToTrove(): Several deposits of 10000 SLSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation', async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
      ...
      // Defaulters 1-4 each withdraw 9999.9 SLSD
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      // B deposits 9999.9 SLSD
      await SLSDToken.transfer(bob, dec(99999, 17), { from: whale })
      ...
      // C deposits 9999.9 SLSD
      await SLSDToken.transfer(carol, dec(99999, 17), { from: whale })
      ...
      // D deposits 9999.9 SLSD
      await SLSDToken.transfer(dennis, dec(99999, 17), { from: whale })
      ...
      assert.equal(await stabilityPool.getCompoundedSLSDDeposit(alice), '0')
      assert.equal(await stabilityPool.getCompoundedSLSDDeposit(alice), '0')
      assert.equal(await stabilityPool.getCompoundedSLSDDeposit(alice), '0')

      assert.equal(await stabilityPoolERC20.getCompoundedSLSDDeposit(alice), '0')
      assert.equal(await stabilityPoolERC20.getCompoundedSLSDDeposit(alice), '0')
      assert.equal(await stabilityPoolERC20.getCompoundedSLSDDeposit(alice), '0')
      // D should retain around 0.9999 SLSD, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedSLSDDeposit(dennis)).toString(), dec(99999, 12)),
        100000
      )
      assert.isAtMost(
        th.getDifference(
    ...
    it('withdrawAssetGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation', async () => {
      ...
      // Defaulters 1-3 each withdraw 24100, 24300, 24500 SLSD (inc gas comp)
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24100, 18), ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(200, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24300, 18), ZERO_ADDRESS),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2, value: dec(200, 'ether') }
      )
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24500, 18), ZERO_ADDRESS),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3, value: dec(200, 'ether') }
      )
      ...
      await borrowerOperations.openTrove(
        ...
        dec(200, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24100, 18), erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      await borrowerOperations.openTrove(
        ...
        dec(200, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24300, 18), erc20.address),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2 }
      )
      await borrowerOperations.openTrove(
        ...
        dec(200, 'ether'),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(24500, 18), erc20.address),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3 }
      )
      ...
      // A, B provide 10k SLSD
      await SLSDToken.transfer(A, dec(10000, 18), { from: whale })
      await SLSDToken.transfer(B, dec(10000, 18), { from: whale })
      ...
      const A_deposit = await stabilityPool.getCompoundedSLSDDeposit(A)
      const B_deposit = await stabilityPool.getCompoundedSLSDDeposit(B)
      assert.equal(A_deposit, '0')
      assert.equal(B_deposit, '0')

      const A_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(A)
      const B_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(B)
      assert.equal(A_depositERC20, '0')
      assert.equal(B_depositERC20, '0')

      // Check SP tracker is zero
      const SLSDinSP_1 = await stabilityPool.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_1, '0')

      const SLSDinSP_1ERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_1ERC20, '0')

      // Check SP SLSD balance is zero
      const SPSLSDBalance_1 = await SLSDToken.balanceOf(stabilityPool.address)
      assert.equal(SPSLSDBalance_1, '0')

      const SPSLSDBalance_1ERC20 = await SLSDToken.balanceOf(stabilityPoolERC20.address)
      assert.equal(SPSLSDBalance_1ERC20, '0')
      ...
      // C, D provide 10k SLSD
      await SLSDToken.transfer(C, dec(10000, 18), { from: whale })
      await SLSDToken.transfer(D, dec(10000, 18), { from: whale })
      ...
      // Check compounded deposits
      const C_deposit = await stabilityPool.getCompoundedSLSDDeposit(C)
      const D_deposit = await stabilityPool.getCompoundedSLSDDeposit(D)
      assert.equal(C_deposit, '0')
      assert.equal(D_deposit, '0')

      const C_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(C)
      const D_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(D)
      assert.equal(C_depositERC20, '0')
      assert.equal(D_depositERC20, '0')

      // Check SP tracker is zero
      const SLSDinSP_2 = await stabilityPool.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_2, '0')

      const SLSDinSP_2ERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_2ERC20, '0')

      // Check SP SLSD balance is zero
      const SPSLSDBalance_2 = await SLSDToken.balanceOf(stabilityPool.address)
      assert.equal(SPSLSDBalance_2, '0')

      const SPSLSDBalance_2ERC20 = await SLSDToken.balanceOf(stabilityPoolERC20.address)
      assert.equal(SPSLSDBalance_2ERC20, '0')
      ...
      // E, F provide 10k SLSD
      await SLSDToken.transfer(E, dec(10000, 18), { from: whale })
      await SLSDToken.transfer(F, dec(10000, 18), { from: whale })
      ...
      // Check compounded deposits
      const E_deposit = await stabilityPool.getCompoundedSLSDDeposit(E)
      const F_deposit = await stabilityPool.getCompoundedSLSDDeposit(F)
      assert.equal(E_deposit, '0')
      assert.equal(F_deposit, '0')

      const E_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(E)
      const F_depositERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(F)
      assert.equal(E_depositERC20, '0')
      assert.equal(F_depositERC20, '0')

      // Check SP tracker is zero
      const SLSDinSP_3 = await stabilityPool.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_3, '0')

      const SLSDinSP_3ERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
      assert.equal(SLSDinSP_3ERC20, '0')

      // Check SP SLSD balance is zero
      const SPSLSDBalance_3 = await SLSDToken.balanceOf(stabilityPool.address)
      assert.equal(SPSLSDBalance_3, '0')

      const SPSLSDBalance_3ERC20 = await SLSDToken.balanceOf(stabilityPoolERC20.address)
      assert.equal(SPSLSDBalance_3ERC20, '0')
    ...
    it('withdrawAssetGainToTrove(): Large liquidated coll/debt, deposits and ETH price', async () => {
      ...
      await borrowerOperations.openTrove(
        ...
        0,
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1, 36), ZERO_ADDRESS),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        ...
        dec(1, 27),
        th._100pct,
        await getOpenTroveSLSDAmount(dec(1, 36), erc20.address),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1 }
      )
      ...
      // Check SLSD balances
      const aliceSLSDBalance = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const aliceSLSDBalanceERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      const aliceExpectedSLSDBalance = web3.utils.toBN(dec(5, 35))

      const aliceSLSDBalDiff = aliceSLSDBalance.sub(aliceExpectedSLSDBalance).abs()
      const aliceSLSDBalDiffERC20 = aliceSLSDBalanceERC20.sub(aliceExpectedSLSDBalance).abs()

      assert.isTrue(aliceSLSDBalDiff.lte(toBN(dec(1, 18)))) // error tolerance of 1e18
      assert.isTrue(aliceSLSDBalDiffERC20.lte(toBN(dec(1, 18)))) // error tolerance of 1e18

      const bobSLSDBalance = await stabilityPool.getCompoundedSLSDDeposit(bob)
      const bobSLSDBalanceERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
      const bobExpectedSLSDBalance = toBN(dec(5, 35))

      const bobSLSDBalDiff = bobSLSDBalance.sub(bobExpectedSLSDBalance).abs()
      const bobSLSDBalDiffERC20 = bobSLSDBalanceERC20.sub(bobExpectedSLSDBalance).abs()

      assert.isTrue(bobSLSDBalDiff.lte(toBN(dec(1, 18))))
      assert.isTrue(bobSLSDBalDiffERC20.lte(toBN(dec(1, 18))))
    ...
    it('withdrawAssetGainToTrove(): Small liquidated coll/debt, large deposits and ETH price', async () => {
      ...
      // Defaulter opens trove with 50e-7 ETH and  5000 SLSD. 200% ICR
      await borrowerOperations.openTrove(
        ZERO_ADDRESS,
        0,
        th._100pct,
      ...
      const aliceSLSDBalance = await stabilityPool.getCompoundedSLSDDeposit(alice)
      const aliceSLSDBalanceERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(alice)
      const aliceExpectedSLSDBalance = toBN('99999999999999997500000000000000000000')
      const aliceSLSDBalDiff = aliceSLSDBalance.sub(aliceExpectedSLSDBalance).abs()
      const aliceSLSDBalDiffERC20 = aliceSLSDBalanceERC20.sub(aliceExpectedSLSDBalance).abs()

      assert.isTrue(aliceSLSDBalDiff.lte(toBN(dec(1, 18))))
      assert.isTrue(aliceSLSDBalDiffERC20.lte(toBN(dec(1, 18))))

      const bobSLSDBalance = await stabilityPool.getCompoundedSLSDDeposit(bob)
      const bobSLSDBalanceERC20 = await stabilityPoolERC20.getCompoundedSLSDDeposit(bob)
      const bobExpectedSLSDBalance = toBN('99999999999999997500000000000000000000')
      const bobSLSDBalDiff = bobSLSDBalance.sub(bobExpectedSLSDBalance).abs()
      const bobSLSDBalDiffERC20 = bobSLSDBalanceERC20.sub(bobExpectedSLSDBalance).abs()

      assert.isTrue(bobSLSDBalDiff.lte(toBN('100000000000000000000')))
      assert.isTrue(bobSLSDBalDiffERC20.lte(toBN('100000000000000000000')))
    })
  })
})
