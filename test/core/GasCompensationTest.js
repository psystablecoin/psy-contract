...
contract('Gas compensation tests', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    community = PSYContracts.communityIssuance
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    debt = 10 SLSD
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 SLSD*/
    ...
    /* ETH:USD price = 200
     ...
     debt = 0 SLSD
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 SLSD*/
    ...
    // debt = 200 SLSD
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  SLSD */
    assert.equal(
  ...
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    debt = 123.45 SLSD
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 SLSD  */
    assert.equal(
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 ETH  
    debt = 2000 SLSD
    -> Expect composite debt = (2000 + 200) = 2200 SLSD  */
    assert.equal(
    ...
    coll = 37.5 ETH  
    debt = 500 SLSD
    -> Expect composite debt = (500 + 200) = 700 SLSD  */
    assert.equal(
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion SLSD
    -> Expect composite debt = (1000000000 + 200) = 1000000200 SLSD  */
    await priceFeed.setPrice('45323545420000000000000')
    assert.isAtMost(
      th.getDifference(
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 SLSD
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 SLSD */
    await priceFeed.setPrice(dec(1, 24))
    assert.equal(
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    // F opens with 1 ETH, 100 SLSD
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: dec(100, 18), extraParams: { from: flyn } })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(2, 18)),
    ...
    // E opens with 4405.45 ETH, 32598.35 SLSD
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraSLSDAmount: B_totalDebt.add(C_totalDebt),
    ...
    console.log((await community.PSYSupplyCaps(stabilityPool.address)).toString())
    console.log((await community.PSYSupplyCaps(stabilityPoolERC20.address)).toString())

    // D, E each provide SLSD to SP
    ...
    const SLSDinSP_0 = await stabilityPool.getTotalSLSDDeposits()
    const SLSDinSP_0ERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
    ...
    // Check SP SLSD has decreased due to the liquidation
    const SLSDinSP_A = await stabilityPool.getTotalSLSDDeposits()
    const SLSDinSP_AERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
    assert.isTrue(SLSDinSP_A.lte(SLSDinSP_0))
    assert.isTrue(SLSDinSP_AERC20.lte(SLSDinSP_0ERC20))
    ...
    // Check SP SLSD has decreased due to the liquidation of B
    const SLSDinSP_B = await stabilityPool.getTotalSLSDDeposits()
    const SLSDinSP_BERC20 = await stabilityPool.getTotalSLSDDeposits()

    assert.isTrue(SLSDinSP_B.lt(SLSDinSP_A))
    assert.isTrue(SLSDinSP_BERC20.lt(SLSDinSP_AERC20))
    ...
    // Check SP SLSD has decreased due to the liquidation of C
    assert.isTrue((await stabilityPool.getTotalSLSDDeposits()).lt(SLSDinSP_B))
    assert.isTrue((await stabilityPoolERC20.getTotalSLSDDeposits()).lt(SLSDinSP_BERC20))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extraSLSDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraSLSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: erin } })
    ...
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(60, 18)),
      extraSLSDAmount: dec(600, 18),
      extraParams: { from: carol },
    })
    await openTrove({
      asset: erc20.address,
      ICR: toBN(dec(80, 18)),
      extraSLSDAmount: dec(1, 23),
      extraParams: { from: dennis },
    })
    await openTrove({
      asset: erc20.address,
    ...
    // D, E each provide 10000 SLSD to SP
    ...
    // Check SP SLSD has decreased due to the liquidation of A
    const SLSDinSP_A = await stabilityPool.getTotalSLSDDeposits()
    const SLSDinSP_AERC20 = await stabilityPoolERC20.getTotalSLSDDeposits()
    assert.isTrue(SLSDinSP_A.lte(SLSDinSP_0))
    assert.isTrue(SLSDinSP_AERC20.lte(SLSDinSP_0ERC20))
    ...
    // Check SP SLSD has decreased due to the liquidation of B
    assert.isTrue((await stabilityPool.getTotalSLSDDeposits()).lt(SLSDinSP_A))
    assert.isTrue((await stabilityPoolERC20.getTotalSLSDDeposits()).lt(SLSDinSP_AERC20))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    await priceFeed.setPrice(dec(400, 18))
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraSLSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(4, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(4, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: erin } })
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraSLSDAmount: B_totalDebt, extraParams: { from: erin } })
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    await openTrove({ ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(118, 16)), extraSLSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraSLSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraSLSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraSLSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraSLSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Check SLSD in SP has decreased
    assert.isTrue((await stabilityPool.getTotalSLSDDeposits()).lt(SLSDinSP_0))
    assert.isTrue((await stabilityPoolERC20.getTotalSLSDDeposits()).lt(SLSDinSP_0ERC20))
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const SLSDinDefaultPool_0 = await defaultPool.getSLSDDebt(ZERO_ADDRESS)
    const SLSDinDefaultPool_0ERC20 = await defaultPool.getSLSDDebt(erc20.address)
    ...
    */
    ...
    // Check SLSD in DefaultPool has decreased
    assert.isTrue((await defaultPool.getSLSDDebt(ZERO_ADDRESS)).gt(SLSDinDefaultPool_0))
    assert.isTrue((await defaultPool.getSLSDDebt(erc20.address)).gt(SLSDinDefaultPool_0))
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 SLSD
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100SLSD
    for (const account of _20_accounts) {
      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extraSLSDAmount: dec(100, 18), extraParams: { from: account, value: collString } })
      await openTrove({
        asset: erc20.address,
