...
class TestHelper {
  ...
  /*
   * given the requested SLSD amount in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, SLSDAmount, asset) {
    if (!asset) asset = this.ZERO_ADDRESS

    const fee = await contracts.troveManagerHelpers.getBorrowingFee(asset, SLSDAmount)
  ...
  static async getEntireCollAndDebt(contracts, account, asset) {
    ...
    const pendingSLSDDebtReward = await contracts.troveManagerHelpers.getPendingSLSDDebtReward(asset, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, SLSDChange) {
    ...
    const fee = SLSDChange.gt(this.toBN('0'))
      ? await contracts.troveManager.getBorrowingFee(SLSDChange)
      : this.toBN('0')
  ...
  static async openTrove_allAccounts_randomETH_ProportionalSLSD(
    minETH,
    maxETH,
    accounts,
    contracts,
  ...
  static async openTrove_allAccounts_randomETH_randomSLSD(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const proportionalSLSD = web3.utils
        .toBN(randSLSDProportion)
  ...
  static async openTrove(
    contracts,
    { asset, assetSent, maxFeePercentage, extraSLSDAmount, upperHint, lowerHint, ICR, extraParams }
  ) {
    if (!asset) asset = this.ZERO_ADDRESS
  ...
  static async withdrawSLSD(
    contracts,
    { asset, maxFeePercentage, SLSDAmount, ICR, upperHint, lowerHint, extraParams }
  ) {
    if (!asset) asset = this.ZERO_ADDRESS
    if (!maxFeePercentage) maxFeePercentage = this._100pct
    if (!upperHint) upperHint = this.ZERO_ADDRESS
    ...
    assert(
      !(SLSDAmount && ICR) && (SLSDAmount || ICR),
      'Specify either SLSD amount or target ICR, but not both'
    )
    ...
    await contracts.borrowerOperations.withdrawSLSD(
      asset,
      maxFeePercentage,
      SLSDAmount,
      upperHint,
  ...
  static async withdrawSLSD_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.withdrawSLSD(this._100pct, amount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async withdrawSLSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSLSD(
        contracts,
        account,
        randSLSDAmount
      )
      ...
      const tx = await contracts.borrowerOperations.withdrawSLSD(
        this._100pct,
        randSLSDAmount,
        upperHint,
        lowerHint,
  ...
  static async repaySLSD_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repaySLSD(amount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async repaySLSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repaySLSD(randSLSDAmount, upperHint, lowerHint, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      ...
      const tx = await stabilityPool.provideToSP(randomSLSDAmount, this.ZERO_ADDRESS, {
        from: account,
      })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
