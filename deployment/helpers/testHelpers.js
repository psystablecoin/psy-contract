...
const TimeValues = {
  ...
  SECONDS_IN_ONE_PSYTH: 60 * 60 * 24 * 30,
  SECONDS_IN_ONE_YEAR: 60 * 60 * 24 * 365,
  MINUTES_IN_ONE_WEEK: 60 * 24 * 7,
  MINUTES_IN_ONE_PSYTH: 60 * 24 * 30,
...
class TestHelper {
  ...
  // Adds the gas compensation (50 SLSD)
  ...
  /*
   * given the requested SLSD amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, SLSDAmount, asset) {
    if (!asset)
      asset = this.ZERO_ADDRESS;

    const fee = await contracts.troveManager.getBorrowingFee(asset, SLSDAmount)
    const compositeDebt = await this.getCompositeDebt(contracts, SLSDAmount, asset)
    return compositeDebt.add(fee)
  }

  /*
   * given the desired total debt, returns the SLSD amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveSLSDAmount(contracts, totalDebt, asset) {
    if (!asset)
      asset = this.ZERO_ADDRESS;
  ...
  static async getAmountWithBorrowingFee(contracts, SLSDAmount, asset) {
    if (!asset)
      asset = this.ZERO_ADDRESS

    const fee = await contracts.troveManager.getBorrowingFee(asset, SLSDAmount)
    return SLSDAmount.add(fee)
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const SLSDAmount = redemptionTx.logs[i].args[1]
        const totalSLSDRedeemed = redemptionTx.logs[i].args[2]
        const totalAssetDrawn = redemptionTx.logs[i].args[3]
        const ETHFee = redemptionTx.logs[i].args[4]

        return [SLSDAmount, totalSLSDRedeemed, totalAssetDrawn, ETHFee]
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const SLSDGasComp = liquidationTx.logs[i].args[4]

        return [liquidatedDebt, liquidatedColl, collGasComp, SLSDGasComp]
      }
    }
  ...
  static getSLSDFeeFromSLSDBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "SLSDBorrowingFeePaid") {
        return (tx.logs[i].args[2]).toString()
      }
    }
    throw ("The transaction logs do not contain an SLSDBorrowingFeePaid event")
  }
  ...
  static async getEntireCollAndDebt(contracts, account, asset) {
    ...
    const pendingSLSDDebtReward = await contracts.troveManager.getPendingSLSDDebtReward(asset, account)
    const entireColl = rawColl.add(pendingAssetReward)
    const entireDebt = rawDebt.add(pendingSLSDDebtReward)
  ...
  static async getCollAndDebtFromWithdrawSLSD(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepaySLSD(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, SLSDChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = SLSDChange.gt(this.toBN('0')) ? await contracts.troveManager.getBorrowingFee(SLSDChange) : this.toBN('0')
    const newColl = entireColl.add(ETHChange)
    const newDebt = entireDebt.add(SLSDChange).add(fee)
  ...
  static async openTrove_allAccounts(accounts, contracts, ETHAmount, SLSDAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, SLSDAmount)

    for (const account of accounts) {
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SLSDAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
  ...
  static async openTrove_allAccounts_randomETH(minETH, maxETH, accounts, contracts, SLSDAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, SLSDAmount)
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SLSDAmount, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomETH_ProportionalSLSD(minETH, maxETH, accounts, contracts, proportion) {
    const gasCostList = []

    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH)
      const proportionalSLSD = (web3.utils.toBN(proportion)).mul(web3.utils.toBN(randCollAmount))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalSLSD)

      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, proportionalSLSD, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomETH_randomSLSD(minETH, maxETH, accounts, contracts, minSLSDProportion, maxSLSDProportion, logging = false) {
    const gasCostList = []
    const price = await contracts.priceFeedTestnet.getPrice()
    ...
    for (const account of accounts) {
      ...
      const randSLSDProportion = this.randAmountInWei(minSLSDProportion, maxSLSDProportion)
      const proportionalSLSD = (web3.utils.toBN(randSLSDProportion)).mul(web3.utils.toBN(randCollAmount).div(_1e18))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalSLSD)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)
      ...
      if (logging && tx.receipt.status) {
        i++
        const ICR = await contracts.troveManager.getCurrentICR(account, price)
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalSLSD} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
  ...
  static async openTrove_allAccounts_randomSLSD(minSLSD, maxSLSD, accounts, contracts, ETHAmount) {
    const gasCostList = []

    for (const account of accounts) {
      const randSLSDAmount = this.randAmountInWei(minSLSD, maxSLSD)
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, randSLSDAmount)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, randSLSDAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_decreasingSLSDAmounts(accounts, contracts, ETHAmount, maxSLSDAmount) {
    const gasCostList = []

    let i = 0
    for (const account of accounts) {
      const SLSDAmount = (maxSLSDAmount - i).toString()
      const SLSDAmountWei = web3.utils.toWei(SLSDAmount, 'ether')
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, SLSDAmountWei)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, SLSDAmountWei, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
      i += 1
    }
  ...
  static async openTrove(contracts, {
    ...
    if (!extraSLSDAmount) extraSLSDAmount = this.toBN(0)
    else if (typeof extraSLSDAmount == 'string') extraSLSDAmount = this.toBN(extraSLSDAmount)
    ...
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, SLSDAmount, asset)
    ...
    const tx = await contracts.borrowerOperations.openTrove(asset, assetSent, maxFeePercentage, SLSDAmount, upperHint, lowerHint, extraParams)
  ...
  static async withdrawSLSD(contracts, {
    asset,
    maxFeePercentage,
    SLSDAmount,
    ICR,
    ...
    assert(!(SLSDAmount && ICR) && (SLSDAmount || ICR), "Specify either SLSD amount or target ICR, but not both")
    ...
    if (ICR) {
      ...
      SLSDAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, SLSDAmount)
    }

    await contracts.borrowerOperations.withdrawSLSD(asset, maxFeePercentage, SLSDAmount, upperHint, lowerHint, extraParams)
  ...
  static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, SLSDAmount) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let SLSDChangeBN = this.toBN(SLSDAmount)

      const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, SLSDChangeBN)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const zero = this.toBN('0')

      let isDebtIncrease = SLSDChangeBN.gt(zero)
      SLSDChangeBN = SLSDChangeBN.abs()

      // Add ETH to trove
      if (ETHChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, SLSDChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: ETHChangeBN })
        // Withdraw ETH from trove
      } else if (ETHChangeBN.lt(zero)) {
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, SLSDChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
      }
  ...
  static async adjustTrove_allAccounts_randomAmount(accounts, contracts, ETHMin, ETHMax, SLSDMin, SLSDMax) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let SLSDChangeBN = this.toBN(this.randAmountInWei(SLSDMin, SLSDMax))
      ...
      if (ETHChangeBN.gt(zero)) {
        ...
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, SLSDChangeBN, isDebtIncrease, lowerHint, upperHint, { from: account })
      }

      const gas = this.gasUsed(tx)
      // console.log(`ETH change: ${ETHChangeBN},  SLSDChange: ${SLSDChangeBN}, gas: ${gas} `)
  ...
  static async withdrawSLSD_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSLSD(contracts, account, amount)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawSLSD(this._100pct, amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawSLSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randSLSDAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawSLSD(contracts, account, randSLSDAmount)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawSLSD(this._100pct, randSLSDAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repaySLSD_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepaySLSD(contracts, account, amount)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repaySLSD(amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repaySLSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randSLSDAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromRepaySLSD(contracts, account, randSLSDAmount)
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repaySLSD(randSLSDAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async redeemCollateral(redeemer, contracts, SLSDAmount, asset, maxFee = this._100pct) {
    if (!asset)
      asset = this.ZERO_ADDRESS

    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, SLSDAmount, asset, maxFee)
    const gas = await this.gasUsed(tx)
    return gas
  }

  static async redeemCollateralAndGetTxObject(redeemer, contracts, SLSDAmount, asset, maxFee = this._100pct) {
    if (!asset)
      asset = this.ZERO_ADDRESS
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts, asset) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randSLSDAmount, asset)
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async performRedemptionTx(redeemer, price, contracts, SLSDAmount, asset, maxFee = 0) {
    if (!asset)
      asset = this.ZERO_ADDRESS;

    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(asset, SLSDAmount, price, 0)
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = []
    for (const account of accounts) {
      const randomSLSDAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.provideToSP(randomSLSDAmount, this.ZERO_ADDRESS, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomSLSDAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.withdrawFromSP(randomSLSDAmount, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async getTimeFromSystemDeployment(psyToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await psyToken.getDeploymentStartTime()
